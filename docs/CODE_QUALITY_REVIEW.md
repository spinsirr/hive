...
PR #34 整合 `main` 的 `7581aba`，保留自动继续队列、在线状态/Thread 读取工具，以及恢复既有任务时跳过 GitHub 克隆凭据请求的修复。`continue-queued-steer` 进入统一 Zod 契约和执行接收流程，新增 hook 测试使用共享测试 fixture。没有恢复重复的 reducer 或路由校验分支。

整合后以相同 Node、pnpm 和隔离 PostgreSQL 环境重新运行完整 release：format（2s）、lint（7s）、design-system（0s）、types（2s）、regression（33s）、integration（38s）、peer-store（1s）、build（20s）全部通过。主 TypeScript 套件为 245 项；新增主干回归覆盖自动续跑、断线重试、失败/恢复后暂停、并发执行授权、多人在线观察和恢复不请求克隆凭据。此记录仍为本地验证，不代表真实模型的生产验收。
