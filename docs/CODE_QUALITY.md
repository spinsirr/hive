...
Check the APIs and types of installed dependencies before building infrastructure. Use Hono for HTTP middleware and Zod request validation, SWR for server-state caching, PartySocket for WebSocket reconnection, and XState for asynchronous client lifecycles with retries and cancellation. Use Octokit for GitHub authentication, typed REST requests and pagination. Use Next/Hono cookie APIs and the MCP and coding harness SDKs for their protocols. Add a library when it removes mechanics we would otherwise maintain; keep Hive's collaboration rules and transactional invariants explicit in the domain and store.
