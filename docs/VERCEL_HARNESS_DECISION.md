...
**Recommendation:** keep Hive's multiplayer room, authorship, annotations,
...
For more than one room, use one unique `sessionId` and one persisted resume
record per room. A stable `sessionId` alone does **not** restore native history:
...
| Conversation | Last 12 room messages rebuilt into every prompt | Native multi-turn session with resume state |
...
- It proved the differentiating product idea first: multiple humans share one
  room, annotate intent, and steer work at explicit boundaries.
...
2. Map one Hive room/repository to a stable harness `sessionId`.
3. Persist the opaque `resumeFrom` state alongside the room when detaching or
   stopping a session.
...
working directory per room and clone into the exact `sessionWorkDir` supplied
...
> moving that execution boundary to Vercel Harness while keeping the shared-room
