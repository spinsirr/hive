...
Current preflight: the actual accessible-label diff and changed-file snapshot remain. The combined test/type/diff checks passed at 05:22 and 06:14 UTC on September 6, but the subsequent text-only turns leave the latest-turn Runs view empty. Execute one fresh bounded check before rehearsal and inspect the real output; a dated pass in these notes or a chat summary is not a current Runs artifact. Do not Reset the task or describe a read-only follow-up as a new implementation. The current Hive source still has the annotation button and its visible idle/queued states, keeping the demo focused on this interaction.
...
5. Inspect the actual Runs exit status, changed-file snapshot, and HEAD diff in both browsers. A read-only follow-up may have no new change; that is not a new implementation. The September 6 05:22 UTC check is retained evidence for its sandbox revision, not proof of a subsequent run. Use only inspectable artifacts and distinguish a recorded result from fresh execution.
...
The live sequence above remains subject to rehearsal. Production verification now also covers discussion-only annotation and explicit promotion during an active run in two accounts, but the agent misidentified the annotation author afterward. The corrected input needs a live retest; do not call the full annotation-to-steer sequence passed yet.
