...
On September 7, the owner authorized pushing and deploying the simpler homepage, retained `/demo`, Reply/Thread UI, whole-thread steering and paired rollback. Deployment and live acceptance of this release are recorded separately in the evidence log. The route below still describes the preceding Annotate flow; after deployment, update that entry to Reply and verify the same discussion → steer → queue sequence before rehearsing. Do not present local fixture checks as two-account production verification. The UI demo is for reviewing the dashboard, not a substitute for the real repository workflow.
