...
On September 7, the owner authorized pushing and deploying the simpler homepage, retained `/demo`, Reply/Thread UI, whole-thread steering and paired rollback. Release `fec2484` received a successful Vercel status and passed the bounded canonical-site UI check below. The script now uses Reply; its discussion → steer → queue sequence still needs rehearsal on this release. Do not present local whole-thread/restore fixtures or read-only live navigation as two-account execution or a real restore. The UI demo reviews the dashboard, not the repository workflow.
