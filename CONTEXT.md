...
Successful turns leave their isolated task environment warm. New conversation or Thread messages renew its 30-minute idle deadline; completing an agent turn renews it too. Polling, presence and edits do not. The provider automatically stops and saves after the deadline, without a connected browser or an active host request. A matching native context is paired with that exact VM's snapshot on the next checkpoint/file read or run admission. Checkpoints are therefore per idle shutdown, not per successful turn; existing checkpoints remain. Failed runs still stop and attempt a recovery snapshot. An unfinished or lost run cannot label a later snapshot with stale native context.
