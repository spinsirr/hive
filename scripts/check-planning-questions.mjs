...
import { createInitialTaskSessionState, reduceTaskSession, appendHiveReply, memberDirectory } from "../src/lib/task-session.ts";
...
const scope = { sessionId: "planning-questions", memberId: "spencer", runId: "run-question" };
let state = reduceTaskSession(createInitialTaskSessionState(1, scope.sessionId), { type: "send-message", actor: "spencer", body: "Ask Maya about spacing" }, 2, members);
state.workspace.liveReply = { id: scope.runId, body: "", startedAt: 2, sequence: 0 };
const realFetch = globalThis.fetch;
const toolNames = [];
let availableTools;
globalThis.fetch = async (url, init) => {
  assert.equal(String(url), "https://hive.test/api/sessions/planning-questions/agent-tools");
  ...
  const response = await handleHiveMcp(request, scope, {
    ...
    control: async () => { throw new Error("Planning must not delegate repository work"); },
  }, createHiveMemory(undefined));
  if (body.method === "tools/list") availableTools = (await response.clone().json()).result.tools.map((tool) => tool.name).sort();
  return response;
};
let connection;
try {
  ...
  assert.equal(state.messages.filter((message) => message.interaction).length, 1);
  assert.equal(question.threadId, undefined);
  assert.equal(question.annotations, undefined);
  state = appendHiveReply(state, "", 4);
  assert.deepEqual(conversationTimelineMessages(state.messages).map((message) => message.id), [state.messages[0].id, question.id], "the question is the reply, without a generated greeting or empty bubble");
  const answer = { type: "answer-question", actor: "maya", messageId: question.id, body: "Compact", clientId: "test-answer" };
  const wrongActor = reduceTaskSession(state, { ...answer, actor: "spencer" }, 5, members);
  assert.equal(wrongActor, state);
  state = reduceTaskSession(state, answer, 6, members);
  assert.equal(state.messages.find((message) => message.id === question.id).interaction.answer.by, "maya");
