...
Client.prototype.query = function (this: Client, ...args: Parameters<Client["query"]>) {
  if (typeof args[0] === "string" && args[0].includes("pg_notify")) {
    return new Promise<void>((resolve) => setTimeout(resolve, 25)).then(() => clientQuery.apply(this, args));
  }
  return clientQuery.apply(this, args);
} as Client["query"];
const latestPresence = new Map<WebSocket, LivePresence>();
async function nextFrame(client: WebSocket, type: string, matches: (presence: LivePresence) => boolean = () => true) {
  for await (const [data] of on(client, "message", { signal: AbortSignal.timeout(10_000) })) {
    const frame = JSON.parse(String(data));
    if (frame.type === type && (type !== "presence" || matches(frame.presence))) return frame;
  }
...
try {
  ...
  for (const { client } of [first, second]) {
    if (latestPresence.get(client)?.activeMembers.length !== 2) await nextFrame(client, "presence", (presence) => presence.activeMembers.length === 2);
    assert.deepEqual(latestPresence.get(client)?.activeMembers, ["github-101", "github-102"]);
  }
  const observer = new SessionEventHub();
  const observation = await observer.readPresence(sessionId);
  assert.deepEqual(observation.activeMembers, ["github-101", "github-102"], "a separate agent-tools instance observes both people without counting itself");
  assert.ok(observation.observedAt <= Date.now());
  for (const { client } of [first, second]) assert.deepEqual(latestPresence.get(client)?.activeMembers, ["github-101", "github-102"]);
  console.log("PASS: independent presence observer reads live members without fake users or durable writes.");
  const typing = [first, second].map(({ client }) => nextFrame(client, "presence", (presence) => presence.typingMembers.includes("github-101")));
  first.client.send(JSON.stringify({ type: "typing", typing: true }));
  await Promise.all(typing);
  console.log("PASS: independent instances exchange presence and attributed typing through real Postgres NOTIFY.");
  const replies = [first, second].map(({ client }) => nextFrame(client, "reply"));
  ...
  const left = nextFrame(second.client, "presence", (presence) => !presence.activeMembers.includes("github-101"));
  first.client.close();
  await once(first.client, "close");
  await left;
  const rejoined = nextFrame(second.client, "presence", (presence) => presence.activeMembers.includes("github-101"));
