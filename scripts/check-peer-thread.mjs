...
try {
  ...
  await waitFor(() => assert.equal(screen.getByRole("button", { name: "Author", exact: true }).disabled, false));
  assert.ok(!screen.queryByRole("textbox", { name: "Answer Hive" }), "option questions do not mount a second composer");
  assert.ok(!screen.queryByRole("button", { name: "Send answer", exact: true }), "option answers need no separate submit action");
  ...
  await waitFor(() => assert.ok(document.activeElement === screen.getByRole("textbox", { name: "Answer Hive" })));
  assert.equal(screen.getByRole("textbox", { name: "Answer Hive" }).rows, 1, "custom answers start as one line and grow with content");
  ...
  let verified;
  const reviewProps = { ...props, message: review, runActive: false, reviewCurrent: true, onViewChanges() {}, onResolveReview: async (_id, revision) => { verified = revision; return true; } };
  view.rerender(createElement(MessageThread, { ...reviewProps, reviewReady: false }));
  assert.ok(screen.getByRole("button", { name: "Mark as reviewed", exact: true }).disabled);
  assert.match(screen.getByRole("group", { name: "Review actions" }).textContent, /Finish pending work and feedback/);
  view.rerender(createElement(MessageThread, { ...reviewProps, reviewReady: false, reviewCurrent: false }));
  assert.match(screen.getByRole("group", { name: "Review actions" }).textContent, /changes are out of date/);
  view.rerender(createElement(MessageThread, { ...reviewProps, reviewReady: false, disabled: true }));
  assert.match(screen.getByRole("group", { name: "Review actions" }).textContent, /Reviewing is paused/);
  ...
  assert.equal(reviewActions.querySelector("p"), null, "a ready review needs no repeated instruction block");
  assert.ok(!reviewActions.className.includes("border"), "review actions have no surrounding card");
  fireEvent.click(screen.getByRole("button", { name: "Mark as reviewed", exact: true }));
  ...
  await waitFor(() => assert.equal(screen.getByRole("button", { name: "Team", exact: true }).disabled, false));
  ...
  const { QuestionAnswer } = await import("../src/components/hive/question-answer.tsx");
  const choiceSubmissions = [];
  let settleChoice;
  const choiceProps = { message: props.message, members, currentMember: "maya", sessionId: "one-click-choice", disabled: false, onAnswer: (_id, submission) => { choiceSubmissions.push(submission); return new Promise((resolve) => { settleChoice = resolve; }); } };
  const choiceView = render(createElement(QuestionAnswer, choiceProps));
  await waitFor(() => assert.equal(screen.getByRole("button", { name: "Author", exact: true }).disabled, false));
  fireEvent.click(screen.getByRole("button", { name: "Author", exact: true }));
  assert.equal(choiceSubmissions.length, 1, "one click submits the choice without a second Answer click");
  assert.equal(choiceSubmissions[0].body, "Author");
  assert.ok(screen.getByRole("button", { name: "Author", exact: true }).disabled);
  assert.ok(screen.getByRole("button", { name: "Team", exact: true }).disabled);
  assert.ok(!screen.queryByRole("button", { name: "Send answer", exact: true }));
  fireEvent.click(screen.getByRole("button", { name: "Author", exact: true }));
  fireEvent.click(screen.getByRole("button", { name: "Team", exact: true }));
  assert.equal(choiceSubmissions.length, 1, "rapid repeat/other choice cannot submit while sending");
  await act(async () => settleChoice(false));
  assert.match(screen.getByRole("status").textContent, /Choose the option again to retry/);
  fireEvent.click(screen.getByRole("button", { name: "Author", exact: true }));
  assert.equal(choiceSubmissions.length, 2);
  assert.equal(choiceSubmissions[0].clientId, choiceSubmissions[1].clientId, "choice retry preserves delivery identity");
  await act(async () => settleChoice(false));
  choiceView.rerender(createElement(QuestionAnswer, { ...choiceProps, disabled: true }));
  fireEvent.click(screen.getByRole("button", { name: "Team", exact: true }));
  assert.equal(choiceSubmissions.length, 2, "read-only choices cannot submit");
  cleanup();
  console.log("PASS: single-click answers submit once, block repeated clicks, retry with the same identity and respect read-only state.");
  const freeTextQuestion = { ...props.message, id: "free-text", interaction: { ...props.message.interaction, options: [] } };
  const submissions = [];
  const freeTextProps = { message: freeTextQuestion, members, currentMember: "maya", sessionId: "compact-free-text", disabled: false, onAnswer: async (_id, submission) => { submissions.push(submission); return false; } };
  ...
  assert.ok(document.activeElement !== screen.getByRole("textbox", { name: "Answer Hive" }), "arrival does not focus a free-text question");
  fireEvent.change(screen.getByRole("textbox", { name: "Answer Hive" }), { target: { value: "Keep drafts private\nuntil shared" } });
  fireEvent.keyDown(screen.getByRole("textbox", { name: "Answer Hive" }), { key: "Enter", keyCode: 229, isComposing: true });
  assert.equal(submissions.length, 0, "IME confirmation does not answer");
  fireEvent.click(screen.getByRole("button", { name: "Send answer", exact: true }));
  await waitFor(() => assert.ok(screen.getByRole("button", { name: "Retry answer", exact: true })));
  cleanup();
  const restoredCustom = { ...freeTextQuestion, interaction: { ...freeTextQuestion.interaction, options: ["Team", "Author"] } };
  const customView = render(createElement(QuestionAnswer, { ...freeTextProps, message: restoredCustom }));
  await waitFor(() => assert.equal(screen.getByRole("textbox", { name: "Answer Hive" }).value, "Keep drafts private\nuntil shared"));
  assert.ok(document.activeElement !== screen.getByRole("textbox", { name: "Answer Hive" }), "restored custom draft is visible without stealing focus");
  fireEvent.click(screen.getByRole("button", { name: "Retry answer", exact: true }));
  await waitFor(() => assert.equal(submissions.length, 2));
  assert.equal(submissions[0].clientId, submissions[1].clientId);
  customView.rerender(createElement(QuestionAnswer, { ...freeTextProps, message: restoredCustom, disabled: true }));
  assert.ok(screen.getByRole("textbox", { name: "Answer Hive" }).disabled);
  assert.ok(screen.getByRole("button", { name: "Team", exact: true }).disabled);
  assert.ok(screen.getByRole("button", { name: "Retry answer", exact: true }).disabled);
  cleanup();
  console.log("PASS: compact free text, IME guard, restored custom draft, retry identity and read-only state remain intact.");
  let copiedText;
