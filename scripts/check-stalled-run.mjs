...
const forbiddenExecution = async () => {
  executionAttempts += 1;
  throw new Error("Recovery must never execute an agent.");
};
mock.module(new URL("../src/lib/hive-runner.ts", import.meta.url).href, {
  namedExports: { runHiveCodingTask: forbiddenExecution },
});
mock.module(new URL("../src/lib/hive-conversation.ts", import.meta.url).href, {
...
try {
  ...
  const {
    ...
    isHiveRunActive,
    canApplyNextSteer,
  } = await import("../src/lib/task-session.ts");
  const { POST, GET } =
    await import("../src/app/api/sessions/[sessionId]/route.ts");
  const { NextRequest } = await import("next/server.js");
  const identities = await Promise.all(
    [901, 902, 903].map((id) =>
      createUserSession({ id, login: `recovery-${id}`, name: `Reviewer ${id}` })
    )
  );
  const [owner, teammate, outsider] = identities;
  const initial = await store.createTaskSession(
    "Lost-run regression fixture",
    owner.member
  );
  ...
  const request = (identity, body) =>
    new NextRequest(`https://hive.test/api/sessions/${id}`, {
      method: body ? "POST" : "GET",
      headers: {
        "Content-Type": "application/json",
        Origin: "https://hive.test",
        ...(identity
          ? { Cookie: `${HIVE_SESSION_COOKIE}=${identity.token}` }
          : {}),
      },
  ...
  const checkpoint = {
    ...
    data: { threadId: "private-recovery-fixture" },
  };
  ...
  await store.applyTaskSessionAction(
    id,
    {
      ...
      installationId: 1,
      repositoryId: 2,
      repositoryUrl: "https://github.com/fixture/recovery.git",
      repositoryName: "fixture/recovery",
  ...
  const child = {
    ...
    runId: oldReplyId,
    kind: "research",
    task: "Inspect this disposable fixture",
    status: "running",
    startedAt: 1,
    result: "",
  };
  await Promise.all([
    ...
    store.checkpointSubagents(id, oldReplyId, {
      runId: oldReplyId,
      sequence: 1,
      tasks: [child],
    }),
  ...
  assert.equal(
    (await read()).workspace.liveReply.body,
  ...
  await store.applyTaskSessionAction(
    ...
    {
      type: "send-message",
      actor: teammate.member.id,
      body: "Queued by the other reviewer",
      clientId: randomUUID(),
    },
    teammate.member
  );
  ...
  assert.equal(
    (await post(undefined, { type: "recover-stalled-run" })).status,
    401
  );
  assert.equal(
    (await post(outsider, { type: "recover-stalled-run" })).status,
    401
  );
  assert.equal(
    (
      await post(owner, {
        type: "recover-stalled-run",
        actor: outsider.member.id,
        now: Date.now() + STALLED_RUN_AFTER_MS,
      })
    ).status,
    200
  );
  assert.deepEqual(
    await read(),
    before,
    "client time and author cannot make a young run recoverable"
  );
  assert.equal((await post(owner, { type: "reset" })).status, 200);
  assert.deepEqual(
    await read(),
    before,
    "reset remains a no-op while the run/queue is active"
  );
  assert.equal(
    (
      await post(owner, {
  ...
  console.log(
    "PASS: real auth/route reject outsiders, premature recovery, active reset and oversized messages without changing stored state."
  );

  // Age only this disposable database fixture. No worker or production state is killed.
  const oldWorkspace = {
    ...before.workspace,
    startedAt: Date.now() - STALLED_RUN_AFTER_MS - 1000,
    sandboxName: "hive-session-recovery-fixture",
    agentSession: { ...before.workspace.agentSession, resumeFrom: checkpoint },
    diff: "+ retained fixture change",
    files: [{ path: "fixture.txt", content: "retained" }],
    commands: [
      {
        command: "fixture check",
        output: { exitCode: 0, output: "fixture result", status: "completed" },
      },
    ],
  ...
  await store.withTaskSubagentControl(id, async (state) => {
    assert.equal(
      state.workspace.agentSession.id,
      aged.workspace.agentSession.id
    );
    assert.doesNotMatch(
      JSON.stringify(state),
      /private-recovery-fixture|resumeFrom|retained fixture change|Partial retained output/
    );
  });
  ...
  assert.equal(
    recovered.version,
    aged.version + 1,
    "two concurrent recoveries record one transition"
  );
  ...
  assert.deepEqual(
    recovered.messages.slice(0, aged.messages.length),
    aged.messages
  );
  assert.equal(
    recovered.messages.filter((message) => message.id === oldReplyId).length,
    1
  );
  assert.equal(recovered.messages.at(-2).body, "Partial retained output");
  assert.equal(
    recovered.messages.at(-2).subagents[0].status,
    "unconfirmed",
    "Lost parent cannot invent a child completion"
  );
  assert.match(
    recovered.messages.at(-1).body,
    /Reviewer marked the run as lost/
  );
  ...
  for (const field of [
    ...
    assert.deepEqual(
      recovered.workspace[field],
      aged.workspace[field],
      `${field} must survive recovery`
    );
  const publicResponse = await GET(request(owner), context);
  assert.equal(publicResponse.status, 200);
  assert.doesNotMatch(
    await publicResponse.text(),
    /private-recovery-fixture|resumeFrom/
  );
  console.log(
    "PASS: two real-Postgres concurrent recoveries preserve discussion, queued authors, partial output, artifacts and private native context exactly once."
  );

  await store.checkpointAgentReply(id, oldReplyId, "STALE TEXT", 99);
  await store.checkpointSubagents(id, oldReplyId, {
    runId: oldReplyId,
    sequence: 99,
    tasks: [{ ...child, status: "completed", result: "STALE CHILD RESULT" }],
  });
  await store.appendHiveReply(id, "STALE RESULT", {
    forReplyId: oldReplyId,
    runError: "stale worker",
  });
  assert.deepEqual(
    await read(),
    recovered,
    "old worker callbacks cannot overwrite recovered state"
  );
  await post(owner, { type: "recover-stalled-run" });
  assert.deepEqual(
    await read(),
    recovered,
    "repeating recovery does not append another error"
  );
  const applied = await Promise.all([
    ...
    store.applyTaskSessionAction(
      id,
      { type: "apply-next-steer", actor: teammate.member.id },
      teammate.member
    ),
  ]);
  ...
  assert.equal(
    continuing.workspace.agentSession.id,
    aged.workspace.agentSession.id
  );
    ...
    tasks: [
      {
        ...child,
        status: "completed",
        result: "STALE CHILD RESULT AFTER APPLY",
      },
    ],
  });
  await store.appendHiveReply(id, "STALE RESULT AFTER APPLY", {
    forReplyId: oldReplyId,
    runError: "stale worker",
  });
  await post(owner, { type: "recover-stalled-run" });
  assert.deepEqual(
    await read(),
    continuing,
    "old callbacks/recovery retry cannot end or corrupt the new run"
  );
  assert.equal(executionAttempts, 0);
  console.log(
    "PASS: late callbacks are fenced; concurrent Apply grants one next run; recovery itself makes zero execution attempts."
  );
  console.log(
