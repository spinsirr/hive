...
registerHooks({
  resolve(specifier, context, next) {
    ...
    return next([".ts", ".tsx"].map((ext) => new URL(`${base.href}${ext}`)).find(existsSync).href, context);
  },
...
let next = "pending";
const client = { reload() {}, async request(_path, init) {
  requests.push(JSON.parse(init.body));
  if (next === "timeout") throw new DOMException("transport timeout", "TimeoutError");
  return next === "confirmed" ? Response.json({ confirmed: true }) : Response.json({ pending: true }, { status: 202 });
} };
const restore = { id: "restore", snapshotId: "snap", startedAt: 1000, retryAfter: 91_000, status: "unconfirmed", by: { id: "owner" } };
const wrapper = ({ children }) => h(HiveClientContext, { value: client }, children);
const tick = async (ms) => act(async () => { mock.timers.tick(ms); });
try {
  const view = renderHook(({ revision, pending }) => useWorkspaceRecovery("task", revision, pending, (value) => received.push(value)), { wrapper, initialProps: { revision: 1, pending: restore } });
  await tick(3000);
  assert.equal(requests.length, 1, "read-only confirmation begins well before the 90-second write retry boundary");
  ...
  assert.equal(requests[1].version, 2, "use the latest revision without restarting the recovery loop");
  assert.match(view.result.current.notice, /Checking again automatically/);
  next = "confirmed";
  await tick(10_000);
  assert.equal(received.length, 1, "a transient timeout does not strand the disabled composer");
  ...
  console.log("PASS: recovery checks promptly, survives a transport timeout, uses the latest revision, confirms once, and stops without retrying writes.");
