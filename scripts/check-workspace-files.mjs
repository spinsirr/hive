...
await writeFile(
  path.join(fixture, "hive", "README.md"),
  "Actual workspace contents, not the saved diff snapshot."
);
...
let codeRuns = 0;
mock.module(
  new URL("../src/lib/codex-subscription-store.ts", import.meta.url).href,
  {
    namedExports: {
      readCodexSubscription: async () => {
        throw new Error("File browsing must not request a model credential");
      },
    },
  }
);
mock.module(new URL("../src/lib/auth-session.ts", import.meta.url).href, {
  namedExports: {
    HIVE_SESSION_COOKIE: "hive_session",
    getSessionMember: async () => member,
  },
});
mock.module(new URL("../src/lib/task-session-store.ts", import.meta.url).href, {
  namedExports: {
    TaskSessionAccessError: class extends Error {},
    withTaskWorkspaceRead: async (_id, read) => read(session),
    startTaskWorkspaceRestore: async () => {
      throw new Error("Restore is outside this read-only check");
    },
    finishTaskWorkspaceRestore: async () => {
      throw new Error("Restore is outside this read-only check");
    },
    recordTaskWorkspaceRestoreSource: async () => {
      throw new Error("Restore is outside this read-only check");
    },
    ...
    appendHiveReply: async (_id, body, options) => {
      session = options?.runResult
        ? reducer.applyHiveRunResult(session, options.runResult)
        : reducer.appendHiveReply(session, body);
...
mock.module(new URL("../src/lib/hive-runner.ts", import.meta.url).href, {
  namedExports: {
    runHiveCodingTask: async (state, actor, steer) => {
      ...
      return {
        sandboxName: "sandbox-qa",
        agentSession: state.workspace.agentSession,
        summary: "Observed the code annotation",
        diff: "",
        files: [],
        commands: [],
        changedFiles: [],
...
mock.module(new URL("../src/lib/hive-conversation.ts", import.meta.url).href, {
  namedExports: {
    runHiveConversation: async () => {
      throw new Error("A code annotation must not start a planning turn");
    },
  },
});
...
try {
  ...
  session.workspace.files = [
    { path: "README.md", content: "WRONG: old snapshot" },
  ];
  const request = (query = "kind=file&path=README.md", id = "files-qa") =>
    GET(
      new NextRequest(`https://hive.example/api/sessions/${id}/files?${query}`),
  ...
  const checkpointRequest = () =>
    checkpoints(
      new NextRequest("https://hive.example/api/sessions/files-qa/checkpoints"),
      { params: Promise.resolve({ sessionId: "files-qa" }) }
    );
  member = null;
  ...
  console.log(
    "PASS: workspace route checks membership, reads the existing worktree, rejects unsafe paths, and never creates a sandbox or starts an agent"
  );
  console.log(
    "PASS: checkpoints are task-scoped, filter deleted/expired snapshots, and neither resume nor stop the sandbox"
  );
  ...
  const post = (body) =>
    POST(
      new NextRequest("https://hive.example/api/sessions/files-qa", {
        method: "POST",
        headers: {
  ...
  const annotation = {
    type: "annotate-code",
    actor: "forged-author",
    clientId: "43725302-b3ef-47b6-a717-70cc459d839f",
    body: "Preserve keyboard focus",
    reference: {
      path: "src/main.tsx",
      startLine: 3,
      endLine: 3,
      quote: "<button>Save</button>",
    },
  };
  admitted = false;
  ...
  assert.equal(
    (
      await post({
        ...annotation,
        reference: { ...annotation.reference, path: "../secret" },
      })
    ).status,
    400
  );
  assert.equal(
    (await post({ ...annotation, clientId: "invalid" })).status,
    400
  );
  ...
  assert.equal(
    (
      await post({
        ...
        annotationId: codeMessage.annotations[0].id,
      })
    ).status,
    200
  );
  ...
  console.log(
    "PASS: real session POST validates code anchors, derives the author from login, deduplicates retries, and gives the code context to the runner only on explicit steer"
  );
  ...
  const reply = {
    ...
    clientId: "311d0669-566f-4080-b8c0-10725e4dc605",
    body: "Also preserve the visible labels.",
  };
  const runsBeforeReply = codeRuns;
  assert.equal((await post(reply)).status, 200);
  const throughReplyId = session.messages
    .find((message) => message.id === agentMessage.id)
    .annotations.at(-1).id;
  assert.equal(
    codeRuns,
    runsBeforeReply,
    "A thread reply never wakes the agent"
  );
  assert.equal(
    (await post({ type: "steer-thread", messageId: agentMessage.id })).status,
    400
  );
  const steerThread = {
    type: "steer-thread",
  ...
  assert.equal(
    codeRuns,
    runsBeforeReply + 1,
    "A retry never executes the same thread twice"
  );
  console.log(
    "PASS: real thread POST keeps replies human-only, requires a reply boundary, enforces membership and server authorship, and executes the frozen thread once"
  );
} finally {
  mock.restoreAll();
