...
for (const scenario of ["background-401", "inference-401", "background-and-inference-401"]) {
  const root = await mkdtemp(path.join(os.tmpdir(), "hive-codex-refresh-"));
  const inferenceFails = scenario !== "background-401";
  const events = [], refreshReplies = [], requests = [];
  let launches = 0, fixtureError;
  const server = createServer(async (req, res) => {
    ...
    if (req.url.includes("/models")) res.end('{"models":[]}');
      ...
      res.statusCode = req.url.endsWith("/settings/user") && scenario !== "inference-401" ? 401 : 403;
      res.end('{"error":{"message":"Fixture-only denial","type":"authentication_error"}}');
    }
  });
  ...
  server.on("upgrade", (req, socket, head) => {
    try {
      ...
      sockets.handleUpgrade(req, socket, head, (ws) => ws.on("message", (raw) => {
        const message = JSON.parse(raw.toString());
        const send = (value) => ws.send(JSON.stringify(value));
        const item = { type: "message", role: "assistant", id: "fixture-message",
        ...
        send({ type: "response.completed", response: { id: "fixture-response", status: "completed",
          output: message.generate === false ? [] : [item], usage: { input_tokens: 10, output_tokens: 5, total_tokens: 15 } } });
      }));
  ...
  try {
    try {
      await runCodexAppServerTurn({
        ...
        turn: { abortSignal: AbortSignal.timeout(12_000), emit: (event) => events.push(event) },
      });
    } catch (error) { caught = error; }
    if (fixtureError) throw fixtureError;
    assert.equal(launches, 1, "No native restart or paid fallback");
    assert.ok(refreshReplies.length > 0, "The real native refresh callback must be exercised");
    assert.ok(refreshReplies.every(reply => reply.error.code === -32000 && !reply.result), "Refresh must be explicitly refused, never supplied a token");
    assert.equal(events.some(event => event.type === "tool-call"), false);
    if (inferenceFails) {
      assert.ok(caught, "Real inference authentication failure must still fail the turn");
      ...
      assert.equal(events.filter(event => event.type === "text-delta").map(event => event.delta).join(""), "HIVE_CODEX_OK");
      assert.equal(events.filter(event => event.type === "finish").length, 1);
    }
    console.log(`PASS: real Codex ${scenario}; refresh refused, ${inferenceFails ? "inference fails closed" : "valid turn completes"}`);
  } finally {
