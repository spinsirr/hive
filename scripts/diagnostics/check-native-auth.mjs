...
assert.ok(
  ...
  "Provide an isolated SDK installation and explicitly authorized seed file"
);
...
let auth;
try {
  auth = parseManagedCodexAuth(
    JSON.parse(await readFile(process.argv[3], "utf8"))
  );
} catch {
  throw new Error("Authorized seed unavailable.");
}
assert.equal(
  codexAuthNeedsRefresh(auth),
  false,
  "Reconnect before testing an expired seed"
);
const home = await mkdtemp(path.join(os.tmpdir(), "hive-native-auth-"));
const app = spawn(
  process.execPath,
  [
    path.join(path.dirname(pkg), require(pkg).bin.codex),
    "app-server",
    "-c",
...
lines.on("line", (line) => {
    ...
    app.stdin.write(
      JSON.stringify({
        ...
        error: {
          code: -32601,
          message:
            "This bounded check cannot refresh credentials or approve actions.",
        },
      }) + "\n"
    );
  ...
  if (
    ...
    !["userMessage", "agentMessage", "reasoning"].includes(msg.params.item.type)
  )
    toolCalls++;
  if (
  ...
  if (!msg.method && pending.has(msg.id)) {
    ...
    if (msg.error)
      item.reject(new Error("Native request failed (details withheld)."));
...
try {
  ...
  await rpc("turn/start", {
    ...
    input: [
      {
        type: "text",
        text: 'Do not call tools or inspect files. Reply with exactly: "Hive native authentication verified."',
        text_elements: [],
      },
    ],
  });
  ...
  console.log(
    "PASS: pinned Codex 0.149.1 accepts ephemeral external tokens and completes one Luna turn; zero tools, no auth.json on disk."
  );
} finally {
  app.stdin.end();
