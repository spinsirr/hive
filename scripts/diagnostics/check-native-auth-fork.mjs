...
const repo = path.join(home, "repo"); await mkdir(repo);
let count = 0;
const server = createServer((req, res) => {
  req.resume();
  count++;
  res.writeHead(429, { "Content-Type": "application/json" });
  res.end(JSON.stringify({ error: { type: "usage_limit_reached", message: "Controlled rejection", plan_type: "pro", resets_at: Math.floor(Date.now() / 1000 + 3600) } }));
});
await new Promise(resolve => server.listen(0, "127.0.0.1", resolve));
const app = spawn(process.execPath, [cli, "app-server"], {
  cwd: repo, env: { PATH: process.env.PATH, CODEX_HOME: home, CODEX_API_KEY: "LOOPBACK_FIXTURE" }, stdio: ["pipe", "pipe", "pipe"], detached: true,
});
...
const config = { model_provider: "fixture", web_search: "disabled", model_providers: { fixture: {
  name: "Local fixture", base_url: `http://127.0.0.1:${server.address().port}/v1`, env_key: "CODEX_API_KEY",
  wire_api: "responses", supports_websockets: false, request_max_retries: 0, stream_max_retries: 0,
} } };
try {
  await rpc("initialize", { clientInfo: { name: "hive-auth-fork-test", version: "0.1.0" }, capabilities: { experimentalApi: true } });
  app.stdin.write(JSON.stringify({ method: "initialized", params: {} }) + "\n");
  const { thread } = await rpc("thread/start", { model: "gpt-5.1-codex-mini", cwd: repo, sandbox: "read-only", approvalPolicy: "never", config });
  await rpc("turn/start", { threadId: thread.id, input: [{ type: "text", text: "Controlled rejected turn", text_elements: [] }] });
  const failed = await Promise.race([completed.promise, new Promise((_, reject) => setTimeout(() => reject(new Error("Turn timeout")), 20_000).unref())]);
  assert.equal(failed.status, "failed"); assert.equal(count, 1);
  const before = await rpc("thread/read", { threadId: thread.id, includeTurns: true });
  assert.equal(before.thread.turns.at(-1).id, failed.id);
  const { thread: fork } = await rpc("thread/fork", { threadId: thread.id, beforeTurnId: failed.id, deferGoalContinuation: true, ephemeral: false, config });
  assert.notEqual(fork.id, thread.id);
  const original = await rpc("thread/read", { threadId: thread.id, includeTurns: true });
  const continuation = await rpc("thread/read", { threadId: fork.id, includeTurns: true });
  assert.equal(original.thread.turns.length, 1);
  assert.equal(continuation.thread.turns.length, 0);
  console.log("PASS: pinned native Codex forks before an empty rejected turn, preserves the original history and does not repeat the rejected prompt.");
} finally {
  app.stdin.end();
