...
// node tests/native/check-native-claude.mjs /path/to/isolated/sdk-install
...
const cliManifest = JSON.parse(
  await readFile(path.join(cliPackage, "package.json"), "utf8")
);
...
const provider = createServer(async (req, res) => {
  try {
    ...
    assert.equal(
      ...
      undefined,
      "OAuth must not silently become API-key auth"
    );
    const chunks = [];
    for await (const chunk of req) chunks.push(chunk);
    const body = JSON.parse(Buffer.concat(chunks));
    if (process.env.HIVE_FIXTURE_DEBUG)
    ...
    if (count === 2)
      assert.match(JSON.stringify(body.messages), /CLAUDE_TOOL_MARKER/);
    if (count === 3) {
      ...
      send({
        ...
        delta: {
          ...
          partial_json: JSON.stringify({
            command: "printf 'CLAUDE_TOOL_MARKER\\n'; exit 7",
            description: "Verify fixture command execution",
          }),
        },
      });
    ...
    if (process.env.HIVE_FIXTURE_DEBUG)
      console.error("fixture failure", req.url, error);
...
try {
  for (const prompt of [
    "Remember HIVE_NATIVE_HISTORY_MARKER. Run only the fixture command.",
    "Keep the earlier context. Only reply now; no commands.",
  ]) {
    const running = query({
      prompt,
      options: {
        ...
        env: {
          CLAUDE_CONFIG_DIR: path.join(workdir, "config"),
    ...
    try {
      for await (const message of running) {
        ...
        if (
          ...
          message.event.delta.text === "Inspecting the task."
        ) {
          assert.equal(
            finishedFirstResponse,
            false,
            "Native output must arrive before message completion"
          );
          partialReceived.resolve();
        }
        ...
        if (message.type === "result") {
          ...
          if (sessionId)
            assert.equal(
              message.session_id,
              sessionId,
              "Resume must preserve the native session identity"
            );
      ...
      assert.equal(
        completed,
        true,
        "The native query must finish successfully"
      );
    } finally {
  ...
  assert.equal(
    toolResults,
    1,
    "A fresh process must not replay the prior Bash command"
  );
  console.log(
    "PASS: pinned Claude uses OAuth bearer auth, emits text before completion, executes one real command and resumes native history without replaying it."
  );
} finally {
  partialReceived.resolve();
