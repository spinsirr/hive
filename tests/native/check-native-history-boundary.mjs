...
// node tests/native/check-native-history-boundary.mjs /path/to/isolated/sdk-install
      ...
      res.end(
        JSON.stringify({
          error: {
            ...
            message: "Controlled foreign encrypted history",
          },
        })
      );
      return;
...
try {
  for (const prompt of [
    "Remember NATIVE-HISTORY-MARKER",
    "Only reply; do not run commands.",
  ]) {
  ...
  assert.equal(
    events.some((e) => e.type === "tool-call" || e.type === "file-change"),
    false
  );
  assert.equal(
    diagnostics.filter((e) => e.attrs.reason === "incompatible_native_history")
      .length,
    1
  );
  console.log(
    "PASS: pinned Codex preserves prior public history, replaces only the incompatible native context, and completes without replaying tools."
  );
} finally {
