...
pool.query = async (...args) => {
  ...
  queries.push({
    ...
    privateHistory: JSON.stringify(result.rows).includes(
      "private-native-history-fixture"
    ),
  });
  return result;
};
...
mock.module("@vercel/functions", {
  namedExports: {
    attachDatabasePool: () => {},
    experimental_upgradeWebSocket: (handler) => {
      const response = new Response(null);
      upgrades.set(response, handler);
      return response;
    },
  },
});
...
function measure() {
  return {
    taskReads: queries.filter(({ text }) => /from "task_sessions"/i.test(text))
      .length,
...
try {
  ...
  const { publicWorkspace } =
    await import("../../src/lib/session/task-session-contract.ts");
  const { GET: live } =
  ...
  for (const [index, member] of members.entries()) {
    await db
      .insert(users)
      .values({ ...member, githubUserId: 101 + index, updatedAt: new Date() });
  ...
  async function connect(index) {
    assert.ok(
      Date.now() < fixtureExpiresAt.getTime(),
      "Local test identity expired while the host was paused; rerun this isolated fixture."
    );
    const response = await live(
      new NextRequest(
        `https://hive.test/api/sessions/${session.sessionId}/live`,
        {
          headers: {
            origin: "https://hive.test",
            cookie: `hive_session=${tokens[index]}`,
          },
        }
      ),
    ...
    assert.equal(
      typeof handle,
      "function",
      "real auth must admit the fixture member"
    );
  ...
  queries.length = 0;
  const changed = viewers.map((client) =>
    nextFrame(
      client,
      (frame) =>
        frame.type === "presence" &&
        frame.presence.typingMembers.includes(members[0].id)
    )
  );
  ...
  for (const frame of updates) {
    assert.equal(
      frame.type,
      "presence",
      "typing must not refresh the full task for every viewer"
    );
    assert.deepEqual(
      frame.presence.activeMembers,
  ...
  assert.equal(
    measure().taskReads,
    0,
    "presence must not read the task row, even if its public response is filtered later"
  );
  assert.equal(
    queries.some(({ text }) => /task_session_presence/i.test(text)),
    false
  );
  console.log(
    "PASS: typing through real authenticated WebSockets; no presence-table or full-task read."
  );

  queries.length = 0;
  notifications.length = 0;
  console.log(
    "Measuring 31 seconds of idle connections, including native ping/pong and one instance renewal…"
  );
  const idleStartedAt = Date.now();
  await new Promise((resolve) => setTimeout(resolve, 31_000));
  console.log("Idle measurement:", {
    elapsedMs: Date.now() - idleStartedAt,
    ...measure(),
    notifications: notifications.length,
    notificationPayloadBytes: notifications.reduce(
  ...
  assert.equal(
    queries.length,
    0,
    "idle probes and unchanged renewal must not read/write any task, auth, or presence tables"
  );
  assert.equal(
    notifications.length,
    1,
    "one shared instance renewal, not one heartbeat per browser"
  );
  console.log(
    "PASS: idle viewers cause zero pooled table queries; only the bounded ephemeral NOTIFY renewal remains."
  );
  ...
  session.workspace.checkpoints = [
    {
      ...
      createdAt: 1,
      result: {
        agentSession: session.workspace.agentSession,
        sandboxName: "fixture-sandbox",
        summary: "Retained checkpoint",
        diff: session.workspace.diff,
  ...
  assert.equal(
    queries.some((item) => item.privateHistory),
    false,
    "public snapshot SQL must remove native history before it leaves Postgres, not after fetching it"
  );
  const stored = await store.getTaskSessionSnapshot(session.sessionId);
  assert.equal(
    stored.session.workspace.agentSession.resumeFrom.data.privateHistory,
    privateHistory,
    "internal recovery readers must retain the original native context"
  );
  assert.equal(
    stored.session.workspace.checkpoints[0].result.agentSession.resumeFrom.data
      .privateHistory,
    privateHistory,
    "paired rollback still needs its private native context"
  );
  const empty = await store.createTaskSession(
  ...
  console.log(
    "PASS: reconnect fetches public data only; the separate recovery read retains native history."
  );

  const revoked = waitFor(viewers[1], "close");
  const remaining = nextFrame(
    viewers[0],
    (frame) =>
      frame.type === "presence" &&
      frame.presence.activeMembers.length === 1 &&
      !frame.presence.typingMembers.length
  );
  await db
    .delete(taskSessionMembers)
    .where(eq(taskSessionMembers.memberId, members[1].id));
  ...
  console.log(
    "PASS: the real live route rechecks database membership on presence; a revoked viewer receives no update."
  );
  const tabRemains = nextFrame(
    anotherTab,
    (frame) => frame.type === "presence"
  );
  viewers[0].close();
  assert.deepEqual((await tabRemains).presence.activeMembers, [members[0].id]);
  console.log(
    "PASS: closing one real tab preserves the same account's remaining connection."
  );
  const expired = waitFor(anotherTab, "close");
  await db
    .update(authSessions)
    .set({ expiresAt: new Date(0) })
    .where(eq(authSessions.userId, members[0].id));
  anotherTab.send(JSON.stringify({ type: "typing", typing: true }));
  assert.equal((await expired)[0], 4401);
  console.log(
    "PASS: an expired real database login closes its existing socket before accepting typing."
  );

  // Agent capability checks use the same real store/route, not a mocked reader.
  const { createHiveToolToken } =
    await import("../../src/server/agents/tools/hive-tool-token.ts");
  const { POST: agentTool } =
    await import("../../src/app/api/sessions/[sessionId]/agent-tools/route.ts");
  process.env.HIVE_INVITE_SECRET =
    "controlled-agent-tool-fixture-not-a-real-secret";
  ...
  console.log(
    "PASS: real tool-context SQL excludes file artifacts and private native history before transferring results."
  );

  const capability = createHiveToolToken(
    toolScope,
    process.env.HIVE_INVITE_SECRET
  );
  const call = (body, token = capability) =>
    agentTool(
      new Request(
        `https://hive.test/api/sessions/${session.sessionId}/agent-tools`,
        {
          method: "POST",
          headers: {
  ...
  const foreign = createHiveToolToken(
    { ...toolScope, sessionId: "different-task" },
    process.env.HIVE_INVITE_SECRET
  );
  ...
  assert.equal(
    queries.length,
    0,
    "invalid capabilities and oversized bodies must fail before querying Postgres"
  );
  const rpc = {
    jsonrpc: "2.0",
    id: 7,
    ...
    params: {
      ...
      arguments: {
        ...
        body: "Should pnpm also apply to CI?",
      },
    },
  };
  ...
  const afterReply = (await store.getTaskSessionSnapshot(session.sessionId))
    .session;
  assert.equal(
    afterReply.messages[0].annotations.length,
    1,
    "a retried tool request must not post a second reply"
  );
  ...
  assert.deepEqual(
    ...
    toolWorkspace,
    "replying must not overwrite the active stream, checkpoints, or code artifacts"
  );
  assert.equal(afterReply.version, toolContext.version + 1);
  console.log(
    "PASS: real signed route stores one attributed Hive reply on retry, with no queue/run/history mutation."
  );
  for (const replay of await Promise.all(
    [70, 71].map((id) => call({ ...rpc, id }))
  ))
    assert.equal((await replay.json()).result.isError, undefined);
  assert.equal(
    (await store.getTaskSessionSnapshot(session.sessionId)).session.messages[0]
      .annotations.length,
    1,
    "a tool retry with a new transport ID must not duplicate the logical reply"
  );
  assert.equal(
    (await store.getTaskSessionSnapshot(session.sessionId)).session.version,
    afterReply.version
  );
  const conflict = await call({
    ...
    params: {
      ...
      arguments: {
        ...
        body: "Different content must not silently disappear",
      },
    },
  });
  ...
  const distinct = await call({
    ...
    params: {
      ...
      arguments: {
        ...rpc.params.arguments,
        key: "test-policy",
        body: "Which checks should run in CI?",
      },
    },
  });
  assert.equal(
    (await distinct.json()).result.isError,
    undefined,
    "reconnecting clients may reuse a transport ID for a distinct logical reply"
  );
  const distinctReplies = (
    await store.getTaskSessionSnapshot(session.sessionId)
  ).session.messages[0].annotations;
  assert.equal(distinctReplies.length, 2);
  assert.equal(distinctReplies[1].body, "Which checks should run in CI?");
  console.log(
    "PASS: reply keys survive changed/reused transport IDs and concurrent retries; conflicting content is rejected and distinct replies remain independent."
  );
  const stale = createHiveToolToken(
    { ...toolScope, runId: "previous-run" },
    process.env.HIVE_INVITE_SECRET
  );
  assert.equal(
    (await (await call({ ...rpc, id: 8 }, stale)).json()).result.isError,
    true
  );
  await db
    .delete(taskSessionMembers)
    .where(eq(taskSessionMembers.memberId, members[0].id));
  assert.equal(
    (await (await call({ ...rpc, id: 9 })).json()).result.isError,
    true
  );
  assert.equal(
    (await store.getTaskSessionSnapshot(session.sessionId)).session.messages[0]
  ...
  console.log(
    "PASS: ended-run and revoked-member capabilities cannot add another reply even before token expiry."
  );
} finally {
