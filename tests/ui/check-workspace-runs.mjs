...
const { RunsPane } =
  await import("../../src/components/hive/workspace/workspace-runs.tsx");
const command = (extra) => ({
  command: "pnpm test",
  exitCode: 0,
...
const render = (commands) =>
  new JSDOM(renderToStaticMarkup(createElement(RunsPane, { commands })));

const maskedError = render([
  command({
    command: "broken-command || true",
...
try {
  ...
  console.log(
    "PASS: exit zero is shown as process evidence, never a test verdict; error output remains intact and escaped."
  );
} finally {
...
try {
  ...
  console.log(
    "PASS: nonzero and missing exit codes remain distinct, with disclosure and original output preserved."
  );
} finally {
  states.window.close();
}
...
try {
  assert.match(
    empty.window.document.body.textContent,
    /No commands in this turn/
  );
...
assert.match(
  unreported.window.document.querySelector("summary").textContent,
  /Exit unavailable/
);
assert.doesNotMatch(
  unreported.window.document.querySelector("summary").textContent,
  /Incomplete|Passed|Exit 0/
);
