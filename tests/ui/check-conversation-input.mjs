...
// Run with: node --experimental-test-module-mocks tests/ui/check-conversation-input.mjs
    ...
    streamText(input) {
      ...
      return {
        fullStream: (async function* () {
          yield {
            type: "text-delta",
            text: "Discussing the selected annotation.",
          };
        })(),
...
const { buildHiveRunInput } =
  await import("../../src/server/agents/hive-prompt.ts");
const { appendHiveReply, createInitialTaskSessionState, reduceTaskSession } =
...
let state = reduceTaskSession(
  createInitialTaskSessionState(1),
  {
    type: "send-message",
    actor: "github-101",
    body: "Discuss the accessible label",
  },
  2,
  members
);
...
console.log(
  "PASS: the pre-repository runner forwards the selected attributed steer to the model"
);
