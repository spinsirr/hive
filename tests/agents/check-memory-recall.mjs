...
import {
  createHiveMemory,
  repositoryMemoryId,
} from "../../src/server/agents/memory/hive-memory.ts";
import { createMemoryRecall } from "../../src/server/agents/memory/hive-memory-recall.ts";
...
const nativeState = {
  ...
  harnessId: "memory-fixture",
  data: {},
};

async function startAgent(
  prepareCall,
  { pause = false, continueFrom, sessionId } = {}
) {
  const turns = [];
  let pendingTurn;
  let continuations = 0;
    ...
    harness: {
      ...
      async doStart({ sessionId, continueFrom }) {
        return {
            ...
            return {
                ...
                for (const name of ["inspect", "check"]) {
                  emit({
                    ...
                    toolCallId: name,
                    toolName: "bash",
                    input: JSON.stringify({ command: name }),
          ...
          async doStop() {
            return { type: "resume-session", ...nativeState };
          },
          async doDestroy() {},
        };
      },
    ...
    sandboxSession: {
      ...
      async run({ command }) {
        assert.match(
          command,
          /^mkdir /,
          "No repository command may run in this fixture"
        );
...
test("a fresh coding turn receives attributed repository memory as context, never as system instructions", async () => {
    ...
    async (url, options) => {
      ...
      return Response.json({
        results: [
          {
            id: "memory-pnpm",
            memory: "Prefer pnpm for this repository.",
            metadata: {
              hive_scope: repositoryMemoryId(scope),
              author_name: "Grace Hopper",
              source_session_id: "earlier-task",
              source_message_id: "human-20",
              source_reply_id: "reply-21",
            },
          },
        ],
      });
  ...
  const { agent, session, turns } = await startAgent(
    createMemoryRecall(memory, scope, "Check the package manager")
  );
  try {
    assert.equal(
      requests.length,
      0,
      "Opening a native session must not recall memory"
    );
    const result = await agent.generate({
      session,
      prompt: "Current task: use npm for this check.",
    });
    ...
    assert.deepEqual(requests, [
      {
        ...
        body: {
          query: "Check the package manager",
          filters: {
    ...
    assert.match(
      turns[0].prompt,
      /"sourceTaskId":"earlier-task"/,
      "the source task must be labeled as a task, not a bare session/message key"
    );
    ...
    assert.ok(
      turns[0].prompt.endsWith("Current task: use npm for this check.")
    );
    assert.equal(
      turns[0].instructions,
      "Execute the current task. Do not push code."
    );
  } finally {
    await session.stop();
  }
...
test("unavailable memory does not block a coding turn, retry, or write", async () => {
  for (const response of [
    ...
    const { agent, session, turns } = await startAgent(
      createMemoryRecall(memory, scope, "Check the label")
    );
    try {
      const result = await agent.generate({
        session,
        prompt: "Check the label, then finish.",
      });
      assert.equal(result.text, "Task finished.");
...
test("a stalled memory request is cancelled within the recall budget and cannot delay coding", async () => {
  ...
  const run = agent.generate({
    session,
    prompt: "Continue without waiting for memory.",
  });
  let watchdog;
  try {
    const result = await Promise.race([
      ...
      new Promise((_, reject) => {
        watchdog = setTimeout(
          () => reject(new Error("Memory blocked coding beyond three seconds")),
          3000
        );
      }),
    ]);
...
test("an empty or foreign repository result leaves the current task unchanged", async () => {
  for (const results of [
    ...
    [
      {
        id: "foreign",
        memory: "OTHER_REPOSITORY_PRIVATE",
        metadata: {
    ...
    try {
      await agent.generate({
        session,
        prompt: "Only work on this repository.",
      });
...
test("recalled context is bounded and cannot inject arbitrary provenance objects", async () => {
  const results = [
    {
      id: "foreign",
      memory: "FOREIGN_SECRET",
      metadata: { hive_scope: "other-repo" },
    },
    {
      id: "m1",
      memory: "A".repeat(4000),
      metadata: {
        hive_scope: repositoryMemoryId(scope),
        author_name: { injected: "UNTRUSTED_AUTHOR_OBJECT" },
        source_session_id: "s".repeat(20_000),
      },
    },
    {
      id: "m2",
      memory: "B".repeat(4000),
      metadata: {
        hive_scope: repositoryMemoryId(scope),
        author_name: "Grace Hopper",
      },
    ...
    {
      id: "m3",
      memory: "C".repeat(4000),
      metadata: { hive_scope: repositoryMemoryId(scope) },
    },
    {
      id: "m4",
      memory: "FOURTH_RESULT",
      metadata: { hive_scope: repositoryMemoryId(scope) },
    },
  ];
  const memory = createHiveMemory("fixture", async () =>
  ...
  const { agent, session, turns } = await startAgent(
    createMemoryRecall(memory, scope, "Repository conventions")
  );
  try {
    await agent.generate({ session, prompt: "Current task stays intact." });
    assert.doesNotMatch(
      turns[0].prompt,
      /FOREIGN_SECRET|FOURTH_RESULT|UNTRUSTED_AUTHOR_OBJECT/
    );
    assert.match(turns[0].prompt, /Grace Hopper/);
    assert.ok(
      turns[0].prompt.length < 6500,
      "Three memories and bounded provenance must fit in a small context block"
    );
...
test("tool steps do not repeat recall; a later fresh turn recalls current memory again", async () => {
  ...
  const memory = createHiveMemory("fixture", async () => {
    ...
    return Response.json({
      results: [
        {
          id: `m${requests}`,
          memory: `RECALLED_VERSION_${requests}`,
          metadata: { hive_scope: repositoryMemoryId(scope) },
        },
      ],
    });
  ...
  const { agent, session, turns } = await startAgent(
    createMemoryRecall(memory, scope, "Package manager")
  );
  try {
    const first = await agent.generate({
      session,
      prompt: "Check the package manager.",
    });
    assert.equal(first.toolCalls.length, 2);
    assert.equal(requests, 1);
    await agent.generate({
      session,
      prompt: "Check the CI package manager too.",
    });
...
test("resuming a suspended native turn does not query memory or start a new prompt", async () => {
  ...
  const stream = await first.agent.stream({
  ...
  await draining;
  assert.equal(requests, 1);
  const resumed = await startAgent(hook, {
    continueFrom: state,
    sessionId: first.session.sessionId,
  });
  try {
    const result = await resumed.agent.continueGenerate({
...
test("disabled memory or unsafe query input never leaves the host", async () => {
  for (const { key, query } of [
    ...
    ...[
      ...
      "postgresql://private-db",
    ].map((query) => ({ key: "fixture", query })),
  ]) {
    let requests = 0;
    const memory = createHiveMemory(key, async () => {
      requests++;
      throw new Error("Unexpected external call");
    });
    const { agent, session, turns } = await startAgent(
      createMemoryRecall(memory, scope, query)
    );
    try {
      await agent.generate({
        session,
        prompt: "Current request stays local to the coding runtime.",
      });
      assert.equal(requests, 0);
      assert.equal(
        turns[0].prompt,
        "Current request stays local to the coding runtime."
      );
    } finally {
      await session.stop();
    }
