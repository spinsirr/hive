// Real HarnessAgent -> native bridge -> Hive reply writer.
...
import {
  consumeAgentText,
  createReplyWriter,
} from "../../src/server/agents/agent-stream.ts";

for (const { failure, unclean } of [
  { failure: false, unclean: false },
  { failure: true, unclean: false },
  { failure: false, unclean: true },
  { failure: true, unclean: true },
]) {
  const checkpoints = [];
  ...
  function launch() {
    ...
    const event = (method, params) =>
      response({
        method,
        params: { threadId: "native-thread", turnId: "native-turn", ...params },
      });
    child.stdin = new Writable({
        ...
        void (async () => {
          ...
          if (
            ...
            assert.equal(
              ...
              "low",
              "Store turn reasoning in thread config so delegated tasks inherit it"
            );
            if (launched === 2) {
              assert.equal(
                request.method,
                "thread/resume",
                "A fresh process must resume the saved native thread"
              );
          ...
          if (request.method === "turn/start") {
            assert.equal(request.params.threadId, "native-thread");
            assert.deepEqual(request.params.input[0], {
              type: "text",
              text: "Controlled native stream",
              text_elements: [],
            });
            response({
              id: request.id,
            ...
            event("turn/completed", {
              turn: {
                ...
                error: failure ? { message: "429 fixture" } : null,
              },
            });
          }
        })().catch((error) => {
      ...
      final(callback) {
        child.stdout.end();
        child.stderr.end();
        child.emit("close", unclean ? 1 : 0, null);
        callback();
      },
    });
    return child;
  ...
  const agent = new HarnessAgent({
    ...
    harness: {
      ...
      harnessId: "native-stream-check",
      builtinTools: {},
      async doStart({ sessionId }) {
        return {
          sessionId,
          isResume: Boolean(savedThread),
          async doPromptTurn({ emit }) {
            const done = runCodexAppServerTurn({
              start: {
                prompt: "Controlled native stream",
                webSearch: false,
                reasoningEffort: "low",
              },
              workdir: "/controlled-native-stream",
  ...
  const sandboxSession = {
    ...
    async run({ command }) {
      assert.match(
        command,
        /^mkdir /,
        "No repository commands may run in this check"
      );
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    assert.equal(Boolean(caught), failure || unclean);
    if (unclean) {
      assert.match(caught.message, /did not shut down cleanly/);
      if (failure) assert.match(caught.cause.message, /429 fixture/);
    } else if (failure) assert.match(caught.message, /429 fixture/);
  }
  assert.deepEqual(
  ...
  assert.equal(
    checkpoints.at(-1).body,
    "第一段，继续。",
    "Completion must not duplicate the streamed prefix"
  );
  assert.deepEqual(
    commands,
    Array(2).fill(
  ...
  console.log(
    `PASS: native deltas and command evidence (turn failed: ${failure}, unclean shutdown: ${unclean}).`
  );
}
