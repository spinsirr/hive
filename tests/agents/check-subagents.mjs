...
import {
  createSubagents,
  subagentSettings,
} from "../../src/server/agents/codex/bridge/subagents.mjs";
...
const settings = {
  ...
  developerInstructions:
    "Preserve authorship and stay in the attached repository.",
  config: {
    model_reasoning_effort: "medium",
    web_search: "live",
    mcp_servers: {
      hive: {
        url: "https://parent.invalid",
        http_headers: { Authorization: "PARENT_SECRET" },
      },
    },
    features: { multi_agent: false, plugins: true },
    skills: {
      config: [{ path: "/fixture/repo/skills/review/SKILL.md", enabled: true }],
    },
...
function fixture(options = {}) {
  ...
  const controller = createSubagents({
    ...
    request: async (method, params) => {
      ...
      if (method === "thread/start")
        return { thread: { id: `thread-${++nextId}` } };
  ...
  const spawn = (kind = "research", requestId = kind) =>
    controller.control({
      action: "spawn",
      kind,
      task: `Inspect the ${kind} evidence`,
      requestId,
    });
  function event(task, method, value = {}) {
    controller.notification({
      method,
      params: { threadId: task.threadId, turnId: task.turnId, ...value },
    });
  }
  function complete(task, status = "completed") {
    event(task, "turn/completed", { turn: { id: task.turnId, status } });
  }
...
test("children inherit the task's configuration and instructions without upgrading the review model", () => {
  ...
  assert.ok(
    child.developerInstructions.startsWith(settings.developerInstructions)
  );
  assert.match(child.developerInstructions, /do not edit files/);
  assert.equal(
    subagentSettings({ ...settings, sandbox: "read-only" }).sandbox,
    "read-only"
  );
  assert.equal(
    settings.config.review_model,
    undefined,
    "Creating a child must not mutate parent configuration"
  );
});
...
test("two independent native threads, bounded to two starts total; same-call retry is idempotent", async () => {
  ...
  assert.deepEqual(
    f.calls.map((call) => call.method).sort(),
    ["review/start", "thread/start", "thread/start", "turn/start"].sort()
  );
  for (const call of f.calls.filter((call) => call.method === "thread/start"))
    assert.deepEqual(
      call.params.config.mcp_servers,
  ...
  assert.equal(
    ...
    undefined,
    "A child turn must use the inherited thread permissions"
  );
  assert.equal(
    turn.params.effort,
    undefined,
    "A child turn must use the inherited reasoning effort"
  );
  const nativeReview = f.calls.find((call) => call.method === "review/start");
  assert.equal(
    nativeReview.params.delivery,
    "inline",
    "The review runs on its new thread, not on the parent"
  );
  assert.ok(
    !JSON.stringify(f.updates).includes("PARENT_SECRET"),
    "Inherited credentials remain private runtime configuration, not shared progress"
  );
...
test("MCP retries of one delegation reuse its child despite different transport request IDs", async () => {
  ...
  try {
    ...
    assert.equal(
      JSON.parse(retry.content[0].text).id,
      JSON.parse(first.content[0].text).id
    );
    assert.equal(
    ...
    f.complete(task);
    assert.equal(
      JSON.parse((await call()).content[0].text).status,
      "completed",
      "a retry cannot relaunch completed work"
    );
    assert.equal(
      (
    ...
    assert.equal(
      f.calls.filter((call) => call.method === "thread/start").length,
      2,
      "distinct work still starts its own child"
    );
  } finally {
...
test("review's internal turn never steals the outer turn or its exitedReviewMode result", async () => {
  const f = fixture();
  const task = await f.spawn("review");
  f.controller.notification({
    method: "turn/started",
    params: { threadId: task.threadId, turn: { id: "internal-review-turn" } },
  });
  f.controller.notification({
    method: "turn/completed",
    params: {
      threadId: task.threadId,
      turn: { id: "internal-review-turn", status: "completed" },
    },
  });
  assert.equal(f.controller.snapshot()[0].status, "running");
  f.event(task, "item/completed", {
  ...
  f.event(task, "item/completed", {
    item: {
      ...
      review: "One actionable finding in src/main.ts:12",
    },
  });
  f.event(task, "item/completed", {
    item: {
      id: "mirror",
      type: "agentMessage",
      text: "One actionable finding in src/main.ts:12",
    },
  });
  f.complete(task);
...
test("interrupt acknowledgement is stopping, not stopped; foreign IDs cannot interrupt the parent", async () => {
  const f = fixture();
  const [a, b] = await Promise.all([f.spawn(), f.spawn("review")]);
  assert.equal(
    (await f.controller.control({ action: "stop", id: a.id })).status,
    "stopping"
  );
  ...
  await assert.rejects(
    f.controller.control({ action: "stop", id: "other-run-task" }),
    /does not belong/
  );
  f.complete(a, "interrupted");
  f.complete(b);
  assert.equal(
    (await f.controller.control({ action: "read", id: a.id })).status,
    "stopped"
  );
  await f.controller.close();
});
...
test("a stop while a thread is starting never starts a model turn", async () => {
  ...
  const spawn = f.spawn();
  const initial = f.controller.snapshot()[0];
  assert.equal(
    (await f.controller.control({ action: "stop", id: initial.id })).status,
    "stopping"
  );
