...
const auth = {
  accessToken: "PRIVATE_ACCESS_NEVER_IN_VM",
  chatgptAccountId: "operator-account",
};
const harness = createHiveCodex(
  { auth: { AI_GATEWAY_API_KEY: "PAID_SECRET" }, reasoningEffort: "max" },
  undefined,
  undefined,
  auth
);
...
await assert.rejects(
  harness.doStart({ sandboxSession: {} }),
  /credential brokering/
);
let transformations;
const result = await harness.doStart({
  sandboxSession: {
    async setRequestTransformations(value) {
...
for (const rule of transformations) {
  ...
  assert.equal(
    rule.transform.headers.Authorization,
    `Bearer ${auth.accessToken}`
  );
  assert.equal(
    rule.transform.headers["ChatGPT-Account-Id"],
    auth.chatgptAccountId
  );
}
assert.ok(
  transformations.every(
    (r) => !/settings|accounts|tasks/.test(r.match.path.exact)
  )
);
console.log(
  "PASS: Codex credentials only enter external egress rules; native settings carry a per-run placeholder, no paid credentials or account-management access."
);
...
const turn = {
  ...
  instructions: "Stable native instructions",
  skills: [],
};
...
assert.equal(
  receivedTurn,
  turn,
  "no Hive server means no advertised Hive tools"
);
const connected = createHiveCodex({
  auth: {},
  mcpServers: { hive: { url: "https://hive.test/agent-tools" } },
});
for (const resumed of [false, true]) {
  const session = await connected.doStart({
    sandboxSession: {},
    ...(resumed ? { resumeFrom: { data: { threadId: "same-history" } } } : {}),
  });
  for (const prompt of [
    turn.prompt,
    ...
    assert.match(
      JSON.stringify(receivedTurn.prompt),
...
console.log(
  "PASS: shared question discovery reaches cold/warm prompts without resetting instructions or native context."
);
