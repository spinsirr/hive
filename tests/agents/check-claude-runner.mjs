...
const resume = {
  ...
  harnessId: "claude-code",
  specificationVersion: "harness-v1",
  data: { claudeSessionId: "native-fixture-id" },
};
const sandbox = {
  async run({ command }) {
    assert.doesNotMatch(
      command,
      /codex-sdk|harness-bootstrap\/codex/,
      "Claude must not repair or start Codex"
    );
    return {
      exitCode: 0,
      stderr: "",
...
const persistent = {
  ...
  tags: { session: "claude-test" },
  persistent: true,
  timeout: 1_800_000,
  expiresAt: new Date(Date.now() + 1_800_000),
  currentSession: () => ({ sessionId: "claude-vm" }),
  keepLastSnapshots: { count: 3 },
  async stop() {
    stopped++;
    return {
      snapshot: {
        id: "saved-native-and-files",
        createdAt: 100,
        status: "created",
      },
    };
  ...
  namedExports: {
    HarnessAgent: class {
      constructor(value) {
        ...
        assert.equal(
          nativeSettings.effort,
          noEffort ? undefined : (explicitEffort ?? "medium"),
          "Shared effort must reach Claude on new and resumed turns"
        );
        assert.deepEqual(nativeSettings.thinking, {
          type: noEffort ? "disabled" : "adaptive",
        });
        assert.equal(
          value.model,
          explicitModel ??
            (subscription ? "claude-sonnet-4-6" : "anthropic/claude-sonnet-4.6")
        );
      ...
      async createSession(options) {
        ...
        await settings.harness.doStart({
          ...options,
          sandboxSession: {
            id: persistent.name,
            async setRequestTransformations() {},
            async addRequestTransformations() {},
          },
        });
        return {
          async stop() {
      ...
      async stream() {
        return {
          fullStream: (async function* () {
            ...
            if (fail)
              throw Object.assign(new Error("authentication_error (fixture)"), {
                statusCode: 401,
              });
...
const saved = {
  HIVE_CLAUDE_SUBSCRIPTION_SCOPE: process.env.HIVE_CLAUDE_SUBSCRIPTION_SCOPE,
  HIVE_CLAUDE_OAUTH_TOKEN: process.env.HIVE_CLAUDE_OAUTH_TOKEN,
...
try {
  ...
  for (subscription of [false, true]) {
    for (fail of [false, true]) {
      ...
      let task = createInitialTaskSessionState(1, "claude-test", {
        createdBy: "owner",
      });
      task = reduceTaskSession(
        task,
        { type: "select-harness", runtime: "claude-code", actor: "owner" },
        2
      );
      task = reduceTaskSession(
        task,
      ...
      task = reduceTaskSession(
        task,
        { type: "set-coding-effort", actor: "owner", effort: "medium" },
        3
      );
      task = reduceTaskSession(
        task,
        { type: "send-message", actor: "owner", body: "Inspect only" },
        4
      );
      if (subscription) {
        process.env.HIVE_CLAUDE_SUBSCRIPTION_SCOPE = JSON.stringify({
      ...
      let result;
      try {
        result = await runHiveCodingTask(task, "owner", undefined, {
          onText: (body) => publicText.push(body),
        });
      } catch (error) {
        assert.equal(fail, true);
        assert.ok(error instanceof HiveAgentError);
        if (subscription)
          assert.equal(error.message, "Reconnect the Claude subscription.");
      ...
      assert.equal(
        result.agentSession.authentication,
        subscription ? "claude-subscription" : "gateway"
      );
      if (fail) assert.equal(result.snapshot.id, "saved-native-and-files");
      else {
        assert.equal(result.snapshot, undefined);
        assert.equal(result.environment.vmId, "claude-vm");
      }
      assert.equal(
        result.commands[0].exitCode,
        null,
        "Never display a synthesized Claude 0/1 as the true process exit code"
      );
      assert.equal(result.commands[0].resultReceived, true);
      assert.equal(result.commands[0].output, "Exit code 7\nREAL_OUTPUT");
      assert.deepEqual(
        publicText,
        fail ? ["Inspecting."] : ["Inspecting.", "Inspecting. Finished."]
      );
      if (!fail) task = applyHiveRunResult(task, result, 5);
      const publicState = publicTaskSessionSnapshot({
        session: task,
        members: [],
        activeMembers: [],
        typingMembers: [],
      ...
      assert.doesNotMatch(
        ...
        /native-fixture-id|sk-ant-oat01-fixture|claude-subscription/
      );
      ...
      if (subscription) {
        ...
        await assert.rejects(
          runHiveCodingTask(task, "owner"),
          /Reconnect the Claude subscription/
        );
        delete process.env.HIVE_CODEX_AUTH_SECRET;
        assert.equal(
          starts,
          before,
          "An auth change must never replay the task using a different account"
        );
      }
    }
  }
  assert.equal(
    stopped,
    2,
    "successful turns keep the VM warm; failed turns stop safely"
  );
  subscription = true;
  fail = false;
  expectedResume = undefined;
  const nativeModels = codingModelOptions("gpt-5.6-luna", true);
  let task = createInitialTaskSessionState(1, "claude-models", {
    createdBy: "owner",
  });
  task = reduceTaskSession(
    task,
  ...
  for (const [modelId, effort] of [
    ...
    assert.equal(
      task.workspace.agentSession,
      history,
      "Same engine keeps native history when the model changes"
    );
    if (effort)
      task = reduceTaskSession(
        task,
        { type: "set-coding-effort", effort, modelId, actor: "owner" },
        5,
        [],
        nativeModels
      );
    ...
    assert.equal(
      task.workspace.codingModel,
      modelId,
      "A successful checkpoint must retain the selected model"
    );
    expectedResume = resume;
  }
  const before = starts;
  ...
  assert.equal(
    starts,
    before,
    "Unsupported model/effort must be rejected before starting a sandbox"
  );
  assert.equal(stopped, 2);
  console.log(
    "PASS: Opus Max, Fable Low/Extra high and Haiku reach the native runtime without losing resume state; invalid selections never start work."
  );
  console.log(
    "PASS: Claude shares the runner, streams only public text, preserves file/native checkpoints after success or failure, resumes exactly and never silently changes authentication."
  );
} finally {
