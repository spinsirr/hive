...
- AI Gateway uses Vercel OIDC in production. Coding defaults to `openai/gpt-5.6-luna` with the existing `low` reasoning setting; `HIVE_CODEX_MODEL` overrides it. Planning stays on `openai/gpt-5-mini`, independently configurable through `HIVE_CHAT_MODEL`. Changing the coding model does not reset the task or change authentication.
- Luna requires the paid AI Gateway credit tier. The September 8 production check was rejected with 403 because this team was still classified as free tier; coding acceptance remains incomplete. This is a Gateway credit entitlement, not a requirement to upgrade the site's Vercel plan. See [Gateway tiers](https://vercel.com/docs/ai-gateway/pricing) and [verification evidence](docs/PRESENTATION_NOTES.md).
