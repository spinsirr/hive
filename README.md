...
The September 6 two-account check verified discussion-only annotations, explicit queuing during a real run, safe-boundary gating, and a second member's ordinary message queuing behind an active steer. It also exposed a failed acceptance condition: Hive confused an annotation's author with its parent message's author. The run input now separates those identities from the promoter and the teammate applying the steer, with regression coverage. A live reverse-author, immediate-steer check passed and survived refresh; the original queued case still requires retesting before the full multiplayer sequence passes.

Still unverified: the corrected original queued annotation case and complete two-account sequence, token-by-token model-delta timing in the browser, and network-loss recovery during generation. Public text has been observed before execution finishes, but that is not a measurement of token cadence. Longer model runs have returned 429; a later successful continuation does not establish that the rate limit is resolved. Runs contains only the latest turn's commands; subsequent text-only replies leave that view empty. Inspect a fresh recorded check before rehearsal instead of treating an earlier chat summary as current command evidence. The [dated evidence log](docs/PRESENTATION_NOTES.md#evidence-log) records those limits; deterministic tests and local fixtures do not replace production checks.
