...
The September 6 two-account check verified discussion-only annotations, explicit queuing during a real run, safe-boundary gating, and a second member's ordinary message queuing behind an active steer. It also exposed a failed acceptance condition: Hive confused an annotation's author with its parent message's author. The run input now separates those identities from the promoter and the teammate applying the steer, with regression coverage; a live retest is still required before the full multiplayer sequence passes.

Still unverified: the corrected live annotation response and complete two-account sequence, token-by-token model-delta timing in the browser, and network-loss recovery during generation. Public text has been observed before execution finishes, but that is not a measurement of token cadence. Longer model runs have returned 429; a later successful continuation does not establish that the rate limit is resolved. The [dated evidence log](docs/PRESENTATION_NOTES.md#evidence-log) records those limits; deterministic tests and local fixtures do not replace production checks.
