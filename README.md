...
Still unverified: token-by-token model-delta timing in the browser, annotation promotion and safe-boundary execution across two accounts, and network-loss recovery during generation. Public text has been observed before execution finishes, but that is not a measurement of token cadence. Longer model runs have returned 429; a later successful continuation does not establish that the rate limit is resolved. The [dated evidence log](docs/PRESENTATION_NOTES.md#evidence-log) records those limits; deterministic tests and local fixtures do not replace production checks.
