...
**Release boundary — September 7:** `396d80f` is deployed, including file caching, wider inline threads and instant conversation entry. Whole-thread execution and paired rollback have local regression coverage, but a new two-account production steer and real restore remain unverified. At the 23:23 UTC preflight, a later text-only reply had left the formal task's latest-turn Runs empty; the earlier passing command is historical evidence, not a currently inspectable run. See the dated [presentation evidence](docs/PRESENTATION_NOTES.md#evidence-log).
...
`pnpm test` also checks native-event translation through the real HarnessAgent and reply writer, including fresh-process resume and partial command retention on failure. An opt-in, zero-paid-model diagnostic runs the pinned real Codex process against a loopback Responses fixture to check native deltas, on-disk history and an actual command.
...
At the September 7, 23:23 UTC preflight, the current Runs view read **No commands in this turn** and the workspace was **Approved**. A matching paired checkpoint had an enabled Restore control, while an older unpaired checkpoint was disabled; neither was restored. Both inspected browsers were signed in as the owner, so this was not a new two-account check. Fresh command evidence, whole-thread/restore acceptance and a timed rehearsal remain submission gates.
