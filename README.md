...
**Release boundary — September 8:** `a13b3a1` is deployed, removing duplicate public context from resumed Codex prompts. The independent two-account coding task now has an inspected two-file change and a real passing combined check: 125 sandbox-revision unit tests, component regressions, TypeScript and diff validation, with exit code 0. Both authenticated accounts received the attributed final response; shared approval and Complete → refresh → Reopen retained the result. This closes the coding/continuation gate, not the recurring provider rate limit or the timed presentation rehearsal. See the dated [presentation evidence](docs/PRESENTATION_NOTES.md#evidence-log).
...
- Resumed Codex turns use the native history without reinjecting public agent replies. Current human input appears once; recent attributed teammate context and explicit steering remain. Fresh/planning turns still receive the discussion they need. This avoids duplicate context, not all history growth.
- GitHub App credentials stay server-side. Sandbox receives a fresh installation token limited to the selected repository.
- AI Gateway uses Vercel OIDC in production. Coding defaults to `openai/gpt-5.1-codex-mini` with the existing `low` reasoning setting; `HIVE_CODEX_MODEL` overrides it. Planning stays on `openai/gpt-5-mini`, independently configurable through `HIVE_CHAT_MODEL`. Changing the coding model does not reset the task or change authentication.
- The owner chose to stay on existing free Gateway credits. GPT-5.1-Codex Mini is listed as [Free Tier eligible](https://vercel.com/ai-gateway/models?freeTier=true&q=gpt-5.1-codex-mini); this means its API usage can use that balance, not that requests are unlimited or have a zero token price. The prior Luna checks returned 403 because Luna required paid Gateway entitlement. No purchase or automatic model fallback is performed. Free-tier rate limits still apply; see [Gateway tiers](https://vercel.com/docs/ai-gateway/pricing) and [verification evidence](docs/PRESENTATION_NOTES.md).
...
Early Gateway 429s left the new coding task incomplete. Real Restore operations returned it to its clean baseline and then to partial work: the diff, dependency directory and recorded commands followed the selected checkpoint, while all three replies and the frozen two-reply steering boundary remained. The failed run did not become approvable or restart automatically. On September 8 at 14:41 UTC, another restore preserved a nonempty pending queue in both accounts. That diagnostic steer was subsequently removed from the queue without deleting its discussion reply.

On **September 8 at 15:01–15:02 UTC**, the same restored task completed one exact `pnpm test && pnpm typecheck && git diff --check` with **Exit 0**. Both accounts inspected the actual output: all **125 unit tests from the sandbox's older checkout**, controlled component/route checks, default-name and caller-override scrolling behavior, and TypeScript passed. This is separate from the deployed application's 135-test local suite. The final answer retained Joseph's behavior requirement and Spinsirr's validation requirement and excluded the later localization discussion. The supervising AI supplied a repair recipe after reproducing the defective regression in an isolated copy; this was an assisted completion, not a clean autonomous first attempt. Two actual model calls returned 200, but the long native context remains and recurring 429 risk is not eliminated. Approval and a two-account Complete → refresh → Reopen cycle preserved the diff and current successful command; no sandbox code was committed or pushed.
