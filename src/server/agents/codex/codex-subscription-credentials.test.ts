...
const now = Date.now();
const auth = parseManagedCodexAuth({
  auth_mode: "chatgpt",
  OPENAI_API_KEY: null,
  tokens: {
    access_token: `fixture.${Buffer.from(JSON.stringify({ exp: Math.floor(now / 1000) + 3600 })).toString("base64url")}.signature`,
...
test("credential envelope is encrypted and bound to task, owner, repository and account", async () => {
  ...
  assert.doesNotMatch(
    sealed,
    /private-refresh|private-id-token|fixture-account/
  );
  ...
  for (const changed of [
    ...
    await assert.rejects(
      openCodexAuth(sealed, { ...binding, ...changed }, secret)
    );
...
test("only short-lived access data reaches the host-side sandbox broker", () => {
  assert.deepEqual(Object.keys(externalCodexTokens(auth)), [
    "accessToken",
    "chatgptAccountId",
  ]);
  assert.doesNotMatch(
    JSON.stringify(externalCodexTokens(auth)),
    /private-refresh|private-id-token/
  );
  assert.equal(codexAuthNeedsRefresh(auth, now), false);
  assert.equal(codexAuthNeedsRefresh(auth, now + 3500_000), true);
  assert.equal(
    codexAuthNeedsRefresh({ ...auth, last_refresh: "invalid" }),
    true
  );
  assert.throws(
    () =>
      parseManagedCodexAuth({ auth_mode: "apikey", OPENAI_API_KEY: "secret" }),
    /managed Codex/
  );
});
