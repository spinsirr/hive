...
import { CODEX_SUBSCRIPTION_MODEL } from "../../../lib/agents/coding-models.ts";
import { refreshCodexSubscription } from "./codex-subscription-refresh.ts";

export class SubscriptionAccessDenied extends Error {}

async function authorizedTask(scope: TaskRunScope) {
  const [task] = await db
    .select({
      ownerId: taskSessions.createdBy,
  ...
  if (!task)
    throw new SubscriptionAccessDenied("This run no longer has access.");
  return task;
}
...
export async function readCodexSubscription(
  ...
  if (bindings.length !== 1)
    throw new Error("Reconnect the platform Codex subscription.");
  const [binding] = bindings;
  if (binding.refreshLock)
    throw new Error("Subscription reconnect or refresh is required.");
  ...
  if (forceRefresh || codexAuthNeedsRefresh(auth)) {
    ...
    const claimed = await db
      .update(codexSubscriptions)
      .set({ refreshLock: lock })
      .where(
        and(
          eq(codexSubscriptions.accountHash, binding.accountHash),
          isNull(codexSubscriptions.refreshLock),
          eq(codexSubscriptions.encryptedAuth, binding.encryptedAuth)
        )
      )
      .returning({ id: codexSubscriptions.accountHash });
    if (!claimed.length)
      throw new Error("Subscription refresh is already in progress.");
    ...
    const saved = await db
      .update(codexSubscriptions)
      .set({ encryptedAuth, refreshLock: null, updatedAt: new Date() })
      .where(
        and(
          eq(codexSubscriptions.accountHash, binding.accountHash),
          eq(codexSubscriptions.refreshLock, lock)
        )
      )
      .returning({ id: codexSubscriptions.accountHash });
    if (!saved.length)
      throw new Error("Subscription refresh could not be retained.");
  }
  ...
  if (
    ...
    JSON.stringify(current.repository) !== JSON.stringify(task.repository)
  )
    throw new SubscriptionAccessDenied("Repository access changed.");
