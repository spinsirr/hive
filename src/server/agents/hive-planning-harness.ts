...
import { createHiveCodex } from "./codex/codex-harness.ts";
import { createHiveClaude } from "./claude/claude-harness.ts";
import { claudeSubscriptionToken } from "./claude/claude-subscription.ts";
...
import type { HiveToolConnection } from "./tools/hive-conversation-tools.ts";
...
export async function runHivePlanningHarness(
  ...
  try {
    ...
    const sessionId =
      task.workspace.agentSession?.id ??
      createAgentSessionId(task.sessionId, task.createdAt);
    const session = await agent.createSession({
      sessionId,
      resumeFrom: task.workspace.agentSession?.resumeFrom,
    });
    try {
      const result = await agent.stream({ session, prompt });
      const summary = (
        await consumePlanningText(result.fullStream, onText)
      ).trim();
      const parked = await environment.park(session);
      return {
        summary,
        sandboxName: parked.sandboxName,
    ...
    throw new HiveAgentError(
      hiveAgentFailureMessage(
        error,
        runtime === "claude-code" ? "claude-subscription" : "codex-subscription"
      ),
      error
    );
  }
