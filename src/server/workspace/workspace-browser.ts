...
} from "../../lib/workspace/workspace-files.ts";
import { workspaceRestoreBlockReason } from "../../lib/workspace/workspace-restore-state.ts";
...
export async function readWorkspace(
  ...
  const workspaceReadScript = await readFile(
    path.join(process.cwd(), "src/server/workspace/workspace-read.mjs"),
    "utf8"
  );
