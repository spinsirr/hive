...
import { publicTaskSessionSnapshot } from "../../lib/session/task-session-snapshot.ts";
import {
...
} from "../../lib/workspace/workspace-restore-state.ts";

function fixture() {
  const session = createInitialTaskSessionState(1, "restore-qa");
  session.repository = {
    provider: "github-app",
    url: "https://github.com/spinsirr/hive.git",
    id: 1,
    name: "spinsirr/hive",
    branch: "main",
    visibility: "private",
    ...
    connectedBy: "spencer",
    connectedAt: 2,
    authorizedByGitHub: { id: 1, login: "spinsirr" },
  };
  let state = session;
  for (let turn = 1; turn <= 4; turn++) {
    state = applyHiveRunResult(
      state,
      {
        snapshot: { id: `snap-${turn}`, createdAt: turn * 10 },
        sandboxName: "hive-session-agent-id",
        agentSession: {
          id: "agent-id",
          runtime: "codex",
          resumeFrom: {
            ...
            harnessId: "codex",
            data: { threadId: `native-${turn}`, secret: "DO NOT EXPOSE" },
          },
        },
        summary: `Finished turn ${turn}`,
        diff: `+ turn ${turn}`,
        files: [{ path: "nav.ts", content: `turn ${turn}` }],
        commands: [{ command: "test", output: `Test ${turn}`, exitCode: 0 }],
        changedFiles: ["nav.ts"],
      },
      turn * 10
    );
...
test("each completed sandbox snapshot is paired with its exact native agent state, retaining a bounded history", () => {
  ...
  assert.deepEqual(
    session.workspace.checkpoints!.map((entry) => entry.id),
    ["snap-4", "snap-3", "snap-2"]
  );
  assert.equal(
    session.workspace.checkpoints![2].result.files[0].content,
    "turn 2"
  );
  assert.deepEqual(
    session.workspace.checkpoints![2].result.agentSession.resumeFrom?.data,
    { threadId: "native-2", secret: "DO NOT EXPOSE" }
  );
  ...
  assert.doesNotMatch(
    ...
    /DO NOT EXPOSE|native-2|native-4/
  );
  assert.equal(
    session.workspace.checkpoints!.length,
    3,
    "Projection never removes stored recovery data"
  );
});
...
test("restore replaces files and native context together, preserves conversation and queue, invalidates review, and is idempotent", () => {
  ...
  session.steeringQueue = [
    {
      id: "keep-this",
      authorId: "maya",
      body: "Check focus later",
      queuedAt: 42,
      source: { kind: "message", messageId: "message" },
      sourceLabel: "Follow-up",
    },
  ];
  const started = beginWorkspaceRestore(
    session,
    { ...request, version: session.version },
    resolveMember("spencer"),
    50
  );
  assert.equal(canApplyNextSteer(started), false);
  assert.equal(
    reduceTaskSession(started, { type: "reset", actor: "spencer" }),
    started
  );
  assert.equal(
  ...
  assert.deepEqual(restored.workspace.agentSession!.resumeFrom!.data, {
    threadId: "native-2",
    ...
    [
      {
        ...
        workspace: {
          ...session.workspace,
          agentSession: { id: "other-agent", runtime: "codex" },
        },
      },
      valid,
    ],
  ];
  for (const [state, input] of rejected)
    assert.throws(() =>
      beginWorkspaceRestore(state, input, resolveMember("maya"), 50)
    );
});
...
test("restoring a failed-run checkpoint keeps its failure and invalidates review", () => {
  ...
  const started = beginWorkspaceRestore(
    failed,
    { ...request, snapshotId: "snap-failed", version: failed.version },
    resolveMember("spencer"),
    60
  );
...
test("a failed or interrupted restore keeps the task fenced and only permits the same bounded operation to retry", () => {
  ...
  assert.throws(() =>
    beginWorkspaceRestore(
      failed,
      { ...request, version: failed.version },
      resolveMember("maya"),
      60
    )
  );
  assert.throws(() =>
    beginWorkspaceRestore(
      failed,
      { ...request, id: "other-operation", version: failed.version },
      resolveMember("maya"),
      100_000
    )
  );
  const retry = beginWorkspaceRestore(
    failed,
    { ...request, version: session.version },
    resolveMember("maya"),
    100_000
...
test("restore evidence survives retries and stale workers cannot record or finish a newer attempt", () => {
  ...
  const recorded = recordWorkspaceRestoreSource(
    started,
    request.id,
    50,
    "vm-before-restore"
  ...
  assert.equal(
    recorded.workspace.restore?.sourceSessionId,
    "vm-before-restore"
  );
  const retry = beginWorkspaceRestore(
    failWorkspaceRestore(recorded, request.id),
    request,
    resolveMember("maya"),
    100_000
  );
  ...
  assert.equal(retry.workspace.restore?.by.id, "spencer");
  assert.throws(
    () => assertWorkspaceRestoreAttempt(retry, request.id, 50),
    /attempt changed/
  );
  assert.throws(
    () => recordWorkspaceRestoreSource(retry, request.id, 50, "wrong-vm"),
    /attempt changed/
  );
  assert.doesNotThrow(() =>
    assertWorkspaceRestoreAttempt(retry, request.id, 100_000)
  );
  assert.equal(
    recordWorkspaceRestoreSource(retry, request.id, 100_000, "another-vm"),
    retry,
    "the original evidence is immutable"
  );
});
