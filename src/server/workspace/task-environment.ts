...
import {
  coldResumeState,
  TASK_ENVIRONMENT_IDLE_MS,
  type TaskEnvironment,
} from "../../lib/workspace/task-environment-policy.ts";
...
export function createTaskEnvironment(
      ...
      return {
        ...
        async doStart(options) {
          ...
          if (!("id" in session) || !("setRequestTransformations" in session))
            throw new Error("A network sandbox is required.");
          network = session;
          ...
          if (
            !sandbox.persistent ||
            sandbox.timeout !== TASK_ENVIRONMENT_IDLE_MS
          ) {
          ...
          const sameVm =
            task.workspace.environment?.vmId ===
            sandbox.currentSession().sessionId;
          return harness.doStart({
            ...options,
            resumeFrom: sameVm
              ? options.resumeFrom
              : coldResumeState(options.resumeFrom),
          });
        },
      };
    },
    async park(session: Pick<HarnessAgentSession, "detach">) {
      if (!sandbox || !network?.setRequestTransformations)
        throw new Error("The task environment was not acquired.");
      ...
      if (resumeFrom.continueFrom)
        throw new Error("Cannot park an unfinished agent turn.");
      return {
        sandboxName: sandbox.name,
        resumeFrom,
        environment: {
          vmId: sandbox.currentSession().sessionId,
          idleUntil: sandbox.expiresAt!.getTime(),
        },
      };
    },
    async stop() {
...
export async function refreshTaskEnvironment(
  ...
  try {
    sandbox = await Sandbox.get({
      name: resolvePersistentSandboxName(task, task.workspace.agentSession.id),
      resume: false,
      signal: AbortSignal.timeout(8_000),
    });
  } catch {
    console.warn("[task-environment] Could not extend idle deadline", {
  ...
  if (current.sessionId !== task.workspace.environment.vmId)
    return task.workspace.environment;
  ...
  if (extension > 0) {
    try {
      await current.extendTimeout(extension, {
        signal: AbortSignal.timeout(8_000),
      });
    } catch {
      ...
      console.warn("[task-environment] Idle extension unavailable", {
        sessionId: task.sessionId,
      });
...
export async function readTaskIdleCheckpoint(
  task: TaskSessionState,
  now = Date.now()
): Promise<SavedWorkspaceCheckpoint | undefined> {
  const pending = task.workspace.idleCheckpoint;
  if (
    !pending ||
    !task.repository ||
    task.workspace.restore ||
    now < (task.workspace.environment?.idleUntil ?? 0)
  )
    return;
  const signal = AbortSignal.timeout(8_000);
  const sandbox = await Sandbox.get({
    name: pending.result.sandboxName,
    resume: false,
    signal,
  });
  assertTaskSandbox(task, sandbox);
  ...
  const saved = snapshots.find(
    (entry) =>
      entry.sourceSessionId === pending.vmId &&
      entry.status === "created" &&
      entry.createdAt >= pending.completedAt &&
      (!entry.expiresAt || entry.expiresAt > now)
  );
  if (!saved) {
    if (sandbox.status !== "running")
      throw new Error(
        "The idle environment is still saving its checkpoint. Try again shortly."
      );
    return;
  }
  return {
    ...
    result: {
      ...
      agentSession: {
        ...pending.result.agentSession,
        resumeFrom: coldResumeState(pending.result.agentSession.resumeFrom),
      },
    },
  };
}
