...
import type { AgentReply, MemberId } from "../../lib/session/task-session.ts";
import type {
  TaskSessionSnapshot,
  LivePresence,
} from "../../lib/session/task-session-contract.ts";
...
export async function subscribeToTaskSession(
  socket: Pick<
    WebSocket,
    "close" | "send" | "bufferedAmount" | "readyState" | "ping" | "terminate"
  > & {
    on: (
  ...
  let stopped = false;
  let subscription:
    Awaited<ReturnType<SessionEventHub["subscribe"]>> | undefined;
  ...
  socket.on("message", (data, isBinary) => {
    ...
    try {
      ...
      if (isBinary || Buffer.byteLength(body) > 128) {
        close(1008);
        return;
      }
  ...
  async function drain() {
    ...
    try {
      while (pending.size && !stopped) {
        const kind = pending.has("snapshot")
          ? "snapshot"
          : pending.has("reply")
            ? "reply"
            : "presence";
        ...
        const event =
          kind === "snapshot"
            ? {
                type: "snapshot",
                snapshot: {
                  ...publicTaskSessionSnapshot(await source.snapshot()),
                  ...presence,
                },
              }
            : kind === "reply"
