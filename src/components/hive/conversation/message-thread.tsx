...
import { ThreadReply } from "@/components/hive/conversation/thread-reply";
...
import { ConversationMessage } from "@/components/hive/conversation/conversation-message";
...
export function MessageThread({
  ...
  const waitingFor =
    question &&
    !question.answer &&
    question.targetMemberId &&
    question.targetMemberId !== currentMember
      ? resolveMember(question.targetMemberId, members).shortName
      : null;
  const deliveredIds = useMemo(
    ...
    [currentMember, replies]
  );
  const send = useCallback(
    (submission: MessageSubmission) => onReply(message.id, submission),
    [message.id, onReply]
  );
  ...
  const previousIndex =
    replies?.findIndex((reply) => reply.id === previous?.throughReplyId) ?? -1;
  const alreadySteered = Boolean(
    previous &&
    !(replies ?? [])
      .slice(previousIndex + 1)
      .some((reply) => reply.role !== "agent")
  );
  const responseInProgress = replies?.some(
    (reply) => reply.deliveryStatus === "streaming"
  );
  const hasQuestionFollowUp =
    ...
    (replies ?? []).some(
      (reply, index, all) =>
        reply.role !== "agent" &&
        index > all.findIndex((entry) => entry.id === question.answer?.replyId)
    );
  ...
  const steerAll = async () => {
    ...
    setError("");
    try {
      if (!(await onSteerThread(message.id, lastReply.id)))
        setError(
          "The thread could not be steered. Reconnect and retry. If the discussion is too long, start a new thread with a summary."
        );
    } finally {
      setSteering(false);
    }
    ...
    >
      <header className="flex h-11 shrink-0 items-center justify-between border-b border-[#ebebeb] px-4">
        <h2 className="flex items-center gap-2 text-xs font-semibold">
          <MessageSquare className="size-3.5 text-[#737373]" /> Thread
        </h2>
        <Button
        ...
        <ConversationContent className="gap-5 px-3 py-5 sm:px-5">
          <article className="border-b border-[#ebebeb] pb-5">
            ...
            <div className="mb-2 flex items-center gap-2 text-xs">
              <span className="grid size-6 shrink-0 place-items-center rounded-full border border-[#dedede] bg-[#fafafa] text-xs font-medium">
                {message.initials}
              </span>
              <span className="font-medium">{message.name}</span>
              <MessageTime className="text-[#999]" message={message} />
            </div>
            <div className="min-w-0 break-words text-sm leading-6">
              {message.role === "agent" ? (
                <AgentResponse>{message.body}</AgentResponse>
              ) : message.codeReference ? (
                <>
                  <p className="break-all text-xs text-[#737373]">
                    {codeReferenceLabel(message.codeReference)}
                  </p>
                  <pre className="mt-2 max-h-52 overflow-auto font-mono text-xs leading-5">
          ...
          {!(replies?.length || requests.length) ? (
            <p className="text-xs text-[#737373]">
              Start a discussion about this message.
            </p>
          ) : null}
          {[
            ...(replies ?? [])
              .filter((reply) => reply.id !== question?.answer?.replyId)
              .map((reply) => ({
                id: reply.id,
                at: reply.createdAt,
                content: (
                  <ThreadReply members={members} reply={reply} showTimestamp />
                ),
              })),
          ...
          {replying && !responseInProgress ? (
            ...
            >
              ...
              />{" "}
              Hive is replying in this thread…
            </p>
          ) : null}
        </ConversationContent>
        <ConversationScrollButton />
      ...
      <footer className="shrink-0 bg-white px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-3 sm:px-5 sm:pb-4">
        {lastReply ? (
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <p className="text-xs text-[#737373]">
              {previous
                ? `${previous.replyCount} ${previous.replyCount === 1 ? "reply" : "replies"} ${previous.status === "queued" ? "queued" : "steered"} by ${resolveMember(previous.requestedBy, members).shortName}`
                : `${replies!.length} ${replies!.length === 1 ? "reply" : "replies"}`}
            </p>
            {hasQuestionFollowUp ? (
              <Button
                aria-label={`${queueing ? "Queue" : "Steer"} entire thread with ${replies!.length} ${replies!.length === 1 ? "reply" : "replies"}`}
                className="h-8 text-xs"
                disabled={
                  disabled || alreadySteered || steering || responseInProgress
                }
                onClick={() => void steerAll()}
                size="sm"
                variant="outline"
              >
                {steering ? (
        ...
        {error ? (
          <p className="mb-2 text-xs text-[#737373]" role="status">
            {error}
          </p>
        ) : null}
        <div className="rounded-[22px] border border-[#e4e4e4] bg-[#fafafa] p-2 focus-within:border-[#c7c7c7]">
          <MentionInput
            ...
            placeholder={
              disabled ? "Replies are paused." : "Reply or @mention a teammate…"
            }
            readOnly={sending}
            value={draft?.body ?? ""}
          />
        ...
        <p className="mt-2 px-1 text-xs text-[#737373]" role="status">
          {draft?.status === "unconfirmed"
            ? "Delivery unconfirmed. Your reply is saved; retry when connected."
            : waitingFor
              ? `Waiting for ${waitingFor}. Your reply is discussion, not an answer.`
              : "Replies stay here. Steer the thread to continue with Hive in the main conversation."}
        </p>
      </footer>
    </section>
  );
