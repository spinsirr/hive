...
import { shouldSubmitMessage } from "@/lib/conversation/message-keyboard";
import type { TeamMember } from "@/lib/session/task-session";
import {
  activeTeammateMention,
  insertTeammateMention,
  matchingTeammates,
  teammateMentionHandle,
} from "@/lib/conversation/teammate-mention";
...
export function MentionInput({
  ...
  disabled,
  readOnly,
  label = "Ask Hive or mention a teammate",
  placeholder,
  maxLength,
  autoFocus = false,
  rows = 3,
  ...
  const suggestions = matchingTeammates(
    members,
    currentMember,
    mention?.query ?? ""
  );
  ...
  return (
    ...
    >
      <Autocomplete.Input
        ...
        aria-multiline="true"
        className={cn(
          "field-sizing-content max-h-[min(12rem,30dvh)] min-h-[5.25rem] min-w-0 flex-1 resize-none bg-transparent px-2 py-2 text-base leading-6 outline-none placeholder:text-[#999] sm:text-sm",
          className
        )}
      ...
      <Autocomplete.Portal>
        ...
        >
          <Autocomplete.Popup className="w-72 max-w-[var(--available-width)] overflow-hidden rounded-lg border border-[#dedede] bg-white p-1 shadow-[0_4px_20px_rgba(0,0,0,0.08)]">
            <Autocomplete.Empty>
              <p className="px-3 py-3 text-xs text-[#737373]">
                {members.some((member) => member.id !== currentMember)
                  ? "No matching teammates"
                  : "Invite a teammate to mention them"}
              </p>
            </Autocomplete.Empty>
            <Autocomplete.List
            ...
            >
              {(member: TeamMember) => (
                <Autocomplete.Item
                  className="flex cursor-default items-center gap-2.5 rounded-md px-2.5 py-2 outline-none data-highlighted:bg-[#f2f2f2]"
                  key={member.id}
                  value={member}
                >
                  <span className="grid size-7 shrink-0 place-items-center rounded-full border border-[#dedede] bg-[#fafafa] text-xs font-medium">
                    {member.initials}
                  </span>
