...
export type AnnotateCode = (reference: CodeReference, submission: MessageSubmission) => Promise<boolean>;

export function CodeAnnotationComposer({ reference, sessionId, memberId, deliveredIds, onSubmit, onClose }: {
  ...
  const { draft, edit, clear, submit } = useMessageDraft(`hive-draft:v1:${sessionId}:${memberId}:code:${codeReferenceLabel(reference)}`, deliveredIds, send);
  ...
  return (
    <form className="shrink-0 border-t border-[#ebebeb] p-3" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
      ...
      <textarea
        ...
        onKeyDown={(event) => { if (event.key === "Enter" && (event.metaKey || event.ctrlKey) && shouldSubmitMessage(event)) { event.preventDefault(); void submit(); } }}
