...
export function MessageThread({ sessionId, message, members, currentMember, disabled, runActive, queue, onClose, onReply, onAnswerQuestion, onSteerReply, onSteerThread, reviewReady = false, reviewCurrent = false, onViewChanges, onResolveReview }: {
  sessionId: string;
  message: ChatMessage;
  members: TeamMember[];
  ...
  onAnswerQuestion?: (messageId: string, submission: MessageSubmission) => Promise<boolean>;
  reviewReady?: boolean;
  ...
  return (
    <section aria-label="Message thread" className="flex h-full min-h-0 flex-col bg-white" onKeyDown={(event) => {
      ...
      <footer className="shrink-0 bg-white px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-3 sm:px-5 sm:pb-4">
        {answering ? <div className="mb-3 flex flex-wrap gap-2">{question?.options.map((option, index) => <Button key={index} size="sm" variant="outline" disabled={disabled || sending} onClick={() => edit(option)}>{option}</Button>)}</div> : null}
        {lastReply ? <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          {hasQuestionFollowUp ? <Button aria-label={`${queueing ? "Queue" : "Steer"} entire thread with ${replies!.length} replies`} className="h-8 text-xs" disabled={disabled || alreadySteered || steering} onClick={() => void steerAll()} size="sm" variant="outline">{steering ? <LoaderCircle className="size-3.5 animate-spin" /> : null}{alreadySteered ? previous?.status === "queued" ? "Thread queued" : "Thread steered" : queueing ? "Queue thread" : "Steer thread"}</Button> : null}
        </div> : null}
        ...
        <div className="rounded-[22px] border border-[#e4e4e4] bg-[#fafafa] p-2 focus-within:border-[#c7c7c7]">
          <MentionInput autoFocus className="block min-h-[3.75rem] w-full px-3 pb-3 pt-2.5" currentMember={currentMember} disabled={disabled || !draft} label={answering ? "Answer Hive" : "Reply in thread"} maxLength={4000} members={members} onChange={edit} onSubmit={() => { if (!disabled) void submit(); }} placeholder={disabled ? "Replies are paused." : answering ? "Choose an option or write your own answer…" : "Reply or @mention a teammate…"} readOnly={sending} value={draft?.body ?? ""} />
          <div className="flex items-center justify-between gap-2 px-1 pb-0.5">
            <span className="text-xs text-[#999]">{answering ? "Answer & continue" : "Reply in thread"}</span>
