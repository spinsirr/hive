...
export type MessageEditTarget = { message: ChatMessage; queuedSteerId?: string };
...
export function MessageEditComposer({ message, queuedSteerId, disabled, onSave, onCancel }: {
  ...
  return (
    <form aria-label="Edit message" id={`message-editor-${message.id}`} className="w-full min-w-0 rounded-xl border border-border bg-muted/30 p-3" onSubmit={(event) => { event.preventDefault(); void save(); }}>
        ...
        <div className="mt-3 flex flex-wrap items-center justify-end gap-2">
          <p className="mr-auto basis-full text-xs leading-5 text-muted-foreground sm:basis-auto sm:flex-1">{queuedSteerId ? "Updates the queued request. Keeps its place in line." : "Keeps edit history. Does not rerun Hive."}</p>
          <Button className="text-xs" disabled={saving} onClick={onCancel} type="button" variant="outline">Cancel</Button>
          <Button className="text-xs" disabled={!canSave} type="submit">{saving ? "Saving…" : "Save changes"}</Button>
        </div>
