...
export function MessageThreadPreview({ message, members, expanded = false, className, onOpen }: {
  message: ChatMessage;
  members: TeamMember[];
  expanded?: boolean;
  className?: string;
  ...
  return (
    <Button data-thread-trigger={message.id} aria-expanded={expanded} aria-label={latest ? `Open thread with ${replies.length} ${replies.length === 1 ? "reply" : "replies"}` : "Open collaboration thread"} className={cn("h-auto w-fit min-w-0 max-w-full flex-col items-stretch gap-1 whitespace-normal rounded-none border-0 border-l-2 border-border bg-transparent px-3 py-1.5 text-left text-xs font-normal text-muted-foreground focus-visible:ring-2 focus-visible:ring-inset active:not-aria-[haspopup]:translate-y-0", expanded && "border-foreground/40 bg-muted/30", className)} onClick={onOpen} type="button" variant="ghost">
      ...
      {!expanded && latest ? <span className="line-clamp-2 break-words text-xs leading-5 [overflow-wrap:anywhere]"><span className="font-medium text-foreground/80">{author}: </span>{preview || (latest.deliveryStatus === "streaming" ? "Replying…" : "Reply")}</span> : null}
    </Button>
  );
}
