...
const reference = {
  ...
  startLine: 3,
  endLine: 5,
  quote: "export const Main = () => {\n  return <button>Save</button>;\n};",
};
const action = {
  type: "annotate-code" as const,
  actor: "maya",
  body: "Add an accessible name, preserving keyboard focus",
...
function withRepository() {
  return reduceTaskSession(
    createInitialTaskSessionState(1),
    {
      ...
      actor: "spencer",
      repositoryUrl: "https://github.com/spinsirr/hive",
...
test("code annotations are durable discussion records, not implicit agent tasks", () => {
  ...
  assert.deepEqual(
    JSON.parse(JSON.stringify(annotated)).messages.at(-1).codeReference,
    reference
  );
  assert.equal(
    reduceTaskSession(annotated, action, 4),
    annotated,
    "retry must not create a second annotation"
  );
  assert.doesNotMatch(
    buildHivePrompt(annotated, "spencer"),
...
for (const running of [false, true]) {
  test(`code annotation ${running ? "queues without interrupting the run" : "starts only on explicit steer"}, with its file, lines, quote and real author`, () => {
    ...
    const promote = {
      ...
      annotationId: message.annotations![0].id,
    };
    ...
    const input = buildHiveRunInput(
      session,
      running ? { type: "apply-next-steer", actor: "spencer" } : promote,
      []
    );
...
test("code references reject traversal and oversized selections; restoring or unattached tasks cannot annotate code", () => {
  assert.equal(
    codeReferenceLabel({ ...reference, startLine: 5 }),
    "src/main.tsx:5"
  );
  for (const change of [
    { path: "../secret" },
    ...
    { endLine: 2 },
    { endLine: 103 },
    { quote: "x".repeat(8_001) },
  ])
    assert.equal(
      codeReferenceSchema.safeParse({ ...reference, ...change }).success,
      false
    );
