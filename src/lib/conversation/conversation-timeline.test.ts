...
test("workspace receipts stay in stored context without masquerading as chat replies", () => {
  const connected: ChatMessage = {
    ...
    initials: "H",
    time: "",
    body: "Ada connected the repository.",
  };
  ...
  const human: ChatMessage = {
    ...
    event: undefined,
    role: "human",
    body: "Please restore the workspace.",
  };
...
test("never hide replies, answers, steers, live work, or different decisions", () => {
  ...
  const variants = [
    ...
    question("two", {
      interaction: {
        ...
        options: ["Yes", "No"],
        targetMemberId: "casey",
        answer: { by: "casey", replyId: "reply", at: 1 },
      },
    }),
    question("two", {
      threadSteer: {
        id: "steer",
        throughReplyId: "reply",
        replyCount: 1,
        requestedBy: "alex",
        requestedAt: 1,
        status: "steered",
  ...
  for (const different of variants)
    assert.deepEqual(conversationTimelineMessages([original, different]), [
      original,
      different,
    ]);
});
...
const turn = (id: string): ChatMessage => ({
  ...
  initials: "H",
  time: "",
  body: "Let's agree on the behavior.",
});
...
test("a tool-created question stays with its run even when the final text arrives later", () => {
  ...
  for (const messages of [
    [card, root],
    [root, card],
    [card, { ...root, status: "streaming" as const }],
  ]) {
    const before = structuredClone(messages);
    ...
    assert.deepEqual(
      messages,
      before,
      "presentation never rewrites history or reply IDs"
    );
  }
});
...
test("grouping never crosses human contributions or combines unrelated agent runs", () => {
  ...
  const human: ChatMessage = {
    ...
    initials: "CA",
    time: "",
    body: "Keep focus visible.",
  };
  for (const messages of [
    [card],
    [card, human, root],
    [card, turn("other")],
    [card, { ...root, status: "error" as const }],
  ]) {
    assert.deepEqual(
      conversationTimelineTurns(messages),
      messages.map((message) => ({ message, requests: [] }))
    );
  }
});
...
test("multiple requests retain their individual Thread identities inside one turn", () => {
  ...
  const second = question("run", {
    id: "peer-run-other",
    body: "What about focus?",
  });
  assert.deepEqual(conversationTimelineTurns([first, second, root]), [
    { message: root, requests: [first, second] },
  ]);
});
