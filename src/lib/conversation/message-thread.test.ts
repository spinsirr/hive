...
import { buildHiveRunInput } from "../../server/agents/hive-prompt.ts";
import {
  appendHiveReply,
...
function discussion(busy = false) {
  ...
  if (!busy)
    session = appendHiveReply(session, "Keep the keyboard focus indicator.", 3);
  ...
  session = reduceTaskSession(
    session,
    {
      ...
      actor: "spencer",
      messageId: parent.id,
      body: "Keep enough space for the labels",
      clientId: "reply-two",
    },
    5
  );
  return {
    session,
    messageId: parent.id,
    throughReplyId: session.messages.at(-1)!.annotations!.at(-1)!.id,
  };
}
...
test("people can reply to a completed agent message without waking Hive or losing earlier replies", () => {
  ...
  assert.deepEqual(
    parent.annotations!.map((reply) => [reply.authorId, reply.body]),
    [
      ["maya", "Make the menu compact"],
      ["spencer", "Keep enough space for the labels"],
    ]
  );
  assert.equal(isHiveRunActive(session), false);
  assert.equal(session.steeringQueue.length, 0);
  const retry = reduceTaskSession(
    session,
    {
      type: "annotate-message",
      actor: "maya",
...
for (const busy of [false, true]) {
  test(`${busy ? "queued" : "immediate"} whole-thread steering preserves all authors and freezes the clicked reply boundary`, () => {
    ...
    assert.equal(
      reduceTaskSession(steered, action, 8),
      steered,
      "Duplicate clicks never add a second steer"
    );
    assert.equal(
      steered.messages.find((message) => message.id === messageId)!.threadSteer!
        .replyCount,
      2
    );
    if (busy) {
      assert.equal(
        steered.workspace.startedAt,
        session.workspace.startedAt,
        "The active run is not interrupted"
      );
      assert.equal(steered.steeringQueue.length, 1);
      steered = appendHiveReply(steered, "Previous run finished", 9);
      steered = reduceTaskSession(
        steered,
    ...
    const input = buildHiveRunInput(
      steered,
      busy ? { type: "apply-next-steer", actor: "spencer" } : action,
      []
    );
    ...
    assert.match(
      input.steer!,
      /handing this discussion back to the main conversation/
    );
...
test("removing a queued thread makes it steerable again without deleting discussion", () => {
  ...
  assert.deepEqual(
    removed.messages.at(-1)!.annotations,
    session.messages.at(-1)!.annotations
  );
...
test("unknown reply boundaries and restoring tasks cannot trigger whole-thread execution", () => {
  ...
  assert.equal(
    reduceTaskSession(
      session,
      {
        ...
        actor: "maya",
        messageId,
        throughReplyId: "not-a-reply",
      },
      6
    ),
    session
  );
  const restoring: TaskSessionState = {
    ...session,
    workspace: {
      ...session.workspace,
      restore: {
  ...
  assert.equal(
    reduceTaskSession(
      restoring,
      { type: "steer-thread", actor: "maya", messageId, throughReplyId },
      6
    ),
    restoring
  );
...
test("an agent acknowledgement alone cannot re-steer the same discussion", () => {
  ...
  const agentReply = finished.messages
  ...
  assert.equal(
    reduceTaskSession(
      finished,
      {
        ...
        actor: "maya",
        messageId,
        throughReplyId: agentReply.id,
      },
      8
    ),
    finished
  );
  const feedback = reduceTaskSession(
    finished,
    {
      type: "annotate-message",
      clientId: crypto.randomUUID(),
  ...
  const lastReply = feedback.messages
    .find((message) => message.id === messageId)!
    .annotations!.at(-1)!;
  const next = reduceTaskSession(
    feedback,
    {
      ...
      actor: "maya",
      messageId,
      throughReplyId: lastReply.id,
    },
    10
  );
  assert.match(next.activeSteer!.body, /Also keep focus visible/);
  assert.match(
    next.activeSteer!.body,
    /Previously shared discussion is context/
  );
});
...
test("whole-thread handoff rejects partial streaming content and freezes inline question context", () => {
  ...
  )!.annotations![0].deliveryStatus = "streaming";
  assert.equal(
    reduceTaskSession(
      partial,
      { type: "steer-thread", actor: "maya", messageId, throughReplyId },
  ...
  session.messages.push({
    ...
    interaction: {
      ...
      runId: "prior-run",
      options: [],
      answer: { replyId: "answer", by: "maya", at: 4 },
    },
    annotations: [
      {
        id: "answer",
