...
import { handleHiveMcp } from "../../server/agents/tools/hive-mcp.ts";
import { createHiveMemory } from "../../server/agents/memory/hive-memory.ts";
import {
...
import { describeHiveContext } from "../../server/agents/tools/hive-tool-context.ts";
...
test("agent can publish a question through its task tools, with choices and a named teammate", async () => {
      ...
      fetch: (input, init) =>
        handleHiveMcp(
          ...
          scope,
          {
            ...
            reply: async () => {
              throw new Error("Use structured input");
            },
            request: async (_scope, request) => {
              const { requestPeerInput } =
                await import("./peer-collaboration.ts");
  ...
  try {
    ...
    assert.equal(
      state.workspace.liveReply?.id,
      "run-one",
      "requesting input does not interrupt the active turn"
    );
    ...
    assert.equal(
      state.messages.filter((m) => m.interaction).length,
      1,
      "retries do not duplicate the question"
    );
  } finally {
    await client.close();
  }
...
test("answering in an explicitly opened Thread continues there once with authorship", () => {
  const { session, messageId } = requestPeerInput(
    working(),
    scope,
    { key: "drafts", prompt: "Who can see drafts?", targetMemberId: "maya" },
    members,
    3
  );
  const action = {
    ...
    actor: "maya",
    messageId,
    replyThreadId: messageId,
    body: "Only the author, until shared.",
    clientId: "answer-one",
  } as const;
  ...
  assert.equal(
    reduceTaskSession(answered, action, 5, members),
    answered,
    "duplicate delivery has no second effect"
  );
  assert.equal(
    reduceTaskSession(session, { ...action, actor: "spencer" }, 4, members),
    session,
    "a different teammate cannot answer a targeted question"
  );
  const completed = applyHiveRunResult(answered, result(), 6);
  const resumed = reduceTaskSession(
    completed,
  ...
  resumed.workspace.liveReply = {
    id: "run-two",
    threadId: hiveReplyThreadId(resumed),
    body: "Checking access",
    sequence: 1,
    startedAt: 7,
  };
  assert.equal(
    conversationMessages(resumed)
  ...
  const finished = applyHiveRunResult(
    resumed,
    { ...result(), summary: "Drafts are private until shared." },
    8
  );
  assert.equal(
    finished.messages.find((m) => m.id === messageId)?.annotations?.at(-1)
      ?.body,
    "Drafts are private until shared."
  );
  assert.equal(
    finished.messages.some((m) => m.id === "run-two"),
    false,
    "result does not escape its thread"
  );
});
...
for (const inThread of [false, true])
  for (const busy of [false, true]) {
    test(`${busy ? "queued" : "immediate"} question answer preserves its ${inThread ? "existing Thread" : "main conversation"} destination`, () => {
      ...
      const requested = requestPeerInput(
        initial,
        scope,
        {
          key: "inline-destination",
      ...
      const question = requested.session.messages.find(
        (message) => message.id === requested.messageId
      )!;
      assert.equal(question.threadId, inThread ? "discussion-root" : undefined);
      assert.equal(
        conversationTimelineMessages(requested.session.messages).some(
          (message) => message.id === question.id
        ),
        !inThread,
        "Thread questions never also appear in the main timeline"
      );
      const session = busy
        ? requested.session
        : applyHiveRunResult(requested.session, result(), 4);
      const action = {
        type: "answer-question",
        actor: "maya",
      ...
      assert.equal(
        reduceTaskSession(
          session,
          { ...action, replyThreadId: "another-task-or-thread" },
          5,
          members
        ),
        session,
        "a client cannot redirect the answer to an unrelated Thread"
      );
      let resumed = reduceTaskSession(session, action, 5, members);
      if (busy)
        resumed = reduceTaskSession(
          applyHiveRunResult(resumed, result(), 6),
          { type: "apply-next-steer", actor: "spencer" },
          7,
          members
        );
      ...
      assert.match(
        input.steer!,
        inThread
          ? /originating Thread discussion-root/
          : /main conversation; do not create a Thread/
      );
      resumed.workspace.liveReply = {
        id: "answer-result",
        threadId,
      ...
      assert.equal(
        inThread
          ? live.find((message) => message.id === threadId)?.annotations?.at(-1)
              ?.body
          : live.find((message) => message.id === "answer-result")?.body,
        "Compact it is."
      );
      const finished = applyHiveRunResult(
        resumed,
        { ...result(), summary: "Compact it is." },
        8
      );
      assert.equal(
        inThread
          ? finished.messages
              ...
              ?.annotations?.at(-1)?.body
          : finished.messages.find((message) => message.id === "answer-result")
              ?.body,
        "Compact it is."
      );
      if (inThread)
        assert.ok(
          !finished.messages.some((message) => message.id === "answer-result"),
          "no escaped duplicate main message"
        );
    });
  }
...
function result(): HiveRunResult {
  return {
    ...
    agentSession: { id: "native-one", runtime: "codex" },
    summary: "Waiting for the team decision.",
    diff: "",
    files: [],
    commands: [],
    changedFiles: [],
...
test("answer continuation preserves a discussion-only request instead of requiring file inspection", () => {
  ...
  initial.messages.push({
    ...
    role: "human",
    time: "12:00",
    body: "Ask me whether I prefer compact or spacious. Do not inspect or change files yet.",
  });
  const requested = requestPeerInput(
    initial,
    scope,
    {
      key: "layout",
      prompt: "Compact or spacious?",
      targetMemberId: "spencer",
    },
    members,
    3
  );
  const answered = reduceTaskSession(
    requested.session,
    {
      type: "answer-question",
      actor: "spencer",
  ...
  assert.match(
    input.steer!,
    /Preserve the original request's scope and restrictions/
  );
...
test("steering discussion on an unanswered question cannot create the same question in a later run", () => {
  ...
  const annotationId = discussion.messages.find(
  ...
  const repeated = requestPeerInput(
    later,
    { ...scope, runId: "discussion-run" },
    request,
    members,
    7
  );
  assert.equal(
    repeated.messageId,
    first.messageId,
    "the stable question key must survive native run boundaries"
  );
  assert.equal(
    repeated.session,
    later,
    "reusing the pending question must not publish an update or enqueue work"
  );
  assert.equal(
    repeated.session.messages.filter((m) => m.interaction?.kind === "question")
      .length,
    1
  );
  ...
  const wrongAnswer = {
    ...
    messageId: first.messageId,
    body: "YES",
    clientId: "wrong-answer",
  } as const;
  assert.equal(
    reduceTaskSession(repeated.session, wrongAnswer, 8, members),
    repeated.session
  );
  const answered = reduceTaskSession(
    repeated.session,
    { ...wrongAnswer, actor: "maya", clientId: "real-answer" },
    8,
    members
  );
  assert.equal(answered.steeringQueue.length, 1);
  const afterAnswer = requestPeerInput(
    answered,
    { ...scope, runId: "discussion-run" },
    request,
    members,
  ...
  assert.equal(
    afterAnswer.session,
    answered,
    "an answered decision cannot be reopened by retrying its key"
  );
  assert.throws(
    () =>
      requestPeerInput(
        later,
        { ...scope, runId: "discussion-run" },
        { ...request, targetMemberId: "spencer" },
        members,
        9
      ),
    /key already used/
  ...
  assert.throws(
    () =>
      requestPeerInput(
        later,
        { ...scope, runId: "discussion-run" },
        { ...request, prompt: "A different decision?" },
        members,
        9
      ),
    /key already used/
  ...
  assert.equal(
    requestPeerInput(
      later,
      { ...scope, runId: "discussion-run" },
      { ...request, key: "new-decision", prompt: "A different decision?" },
      members,
      9
    ).session.messages.filter((m) => m.interaction?.kind === "question").length,
    2
  );
});
...
test("review keys still produce a new revision-bound review in a later run", () => {
  ...
  next.workspace.liveReply = {
    id: "next-change",
    body: "",
    sequence: 0,
    startedAt: 5,
  };
  const second = requestPeerInput(
    next,
    { ...scope, runId: "next-change" },
    request,
    members,
    6
  );
  ...
  assert.equal(
    second.session.messages.at(-1)?.interaction?.runId,
    "next-change"
  );
});

test("question metadata survives the discussion window without exposing queued answers", () => {
  const first = requestPeerInput(
    working(),
    scope,
    { key: "decision", prompt: "A decision?", targetMemberId: "maya" },
    members,
    3
  );
  const answered = reduceTaskSession(
  ...
  assert.deepEqual(context.questions, [
    {
      messageId: first.messageId,
      threadId: null,
      key: "decision",
...
for (const steerKind of ["thread", "reply"])
  test(`${steerKind}: main-conversation result updates the source review, and only a human can resolve current evidence`, () => {
    const { session, messageId } = requestPeerInput(
      working(),
      scope,
      {
    ...
    assert.equal(
      reduceTaskSession(session, resolve, 4, members),
      session,
      "cannot approve a running workspace"
    );
    const ready = applyHiveRunResult(
      session,
      { ...result(), diff: "+ private drafts", changedFiles: ["access.ts"] },
      5
    );
    assert.equal(
      ready.messages.find((m) => m.id === messageId)?.interaction?.revision,
      "run-one"
    );
    for (const actor of ["spencer", "maya"]) {
      const retiredApproval = JSON.parse(
        JSON.stringify({ type: "advance-run", actor })
      );
      assert.equal(
        reduceTaskSession(ready, retiredApproval, 6, members),
        ready,
        "neither reviewer nor teammate can bypass the review through global approval"
      );
    }
    assert.equal(
      reduceTaskSession(ready, { ...resolve, actor: "spencer" }, 6, members),
      ready
    );
    assert.equal(
      reduceTaskSession(ready, { ...resolve, revision: "stale" }, 6, members),
      ready
    );
    const resolved = reduceTaskSession(ready, resolve, 6, members);
    assert.equal(
      resolved.messages.find((m) => m.id === messageId)?.interaction?.resolved
        ?.by,
      "maya"
    );
    ...
    const discussion = reduceTaskSession(
      ready,
      {
        ...
        actor: "maya",
        messageId,
        body: "Check shared drafts too",
        clientId: "feedback-one",
      },
      6,
      members
    );
    assert.equal(
      reduceTaskSession(discussion, resolve, 7, members),
      discussion,
      "unaddressed discussion cannot be silently resolved"
    );
    const replyId = discussion.messages
      .find((m) => m.id === messageId)!
      .annotations!.at(-1)!.id;
    const revised = reduceTaskSession(
      discussion,
      steerKind === "thread"
        ? {
            type: "steer-thread",
    ...
    assert.equal(
      failed.messages.find((m) => m.id === messageId)?.interaction?.status,
      "unavailable"
    );
    assert.equal(
      failed.messages.find((m) => m.id === messageId)?.interaction?.revision,
      undefined
    );
    const verify = applyHiveRunResult(
      revised,
      { ...result(), diff: "+ private and shared drafts" },
      9
    );
    assert.equal(
      verify.messages.find((m) => m.id === "run-two")?.role,
      "agent"
    );
    assert.deepEqual(
      verify.messages.find((m) => m.id === messageId)?.annotations,
      revised.messages.find((m) => m.id === messageId)?.annotations
    );
    assert.equal(
      verify.messages.find((m) => m.id === messageId)?.interaction?.revision,
      "run-two"
    );
    assert.equal(
      reduceTaskSession(verify, resolve, 10, members),
      verify,
      "an old review cannot approve the revision"
    );
    assert.equal(
      reduceTaskSession(
        verify,
        { ...resolve, revision: "run-two" },
        10,
        members
      ).messages.find((m) => m.id === messageId)?.interaction?.resolved?.by,
      "maya"
    );
  });
...
test("interrupted runs preserve the answer and thread, but cannot auto-continue or expose a ready review", () => {
  const { session, messageId } = requestPeerInput(
    working(),
    scope,
    { kind: "review", key: "review", prompt: "Check access" },
    members,
    3
  );
  ...
  const question = requestPeerInput(
    working(),
    scope,
    { key: "decision", prompt: "Team or private?" },
    members,
    3
  );
  const answered = reduceTaskSession(
  ...
  assert.equal(
    reduceTaskSession(
      lost,
      {
        ...
        actor: "maya",
        steerId: lost.steeringQueue[0].id,
      },
      6,
      members
    ),
  ...
  assert.equal(
    interrupted.messages
      .find((m) => m.id === question.messageId)
      ?.annotations?.at(-1)?.deliveryStatus,
    "error"
  );
  assert.equal(
    interrupted.messages
...
test("automatic continuation cannot consume a different queue item, run revoked input, or replay a restored queue", () => {
  ...
  const action = {
    type: "continue-queued-steer",
    actor: "spencer",
    steerId: idle.steeringQueue[0].id,
  } as const;
  assert.equal(
    reduceTaskSession(
      idle,
      { ...action, steerId: "old-queue-item" },
      6,
      members
    ),
    idle
  ...
  assert.equal(
    reduceTaskSession(idle, action, 6, [memberDirectory.spencer]),
    idle,
    "removed answer author cannot direct another run"
  );
  const restored = {
    ...idle,
    workspace: {
  ...
  assert.doesNotMatch(
    JSON.stringify(describeHiveContext(queuedContext)),
    /Private/
  );
});
...
test("review completion covers only the feedback captured by that run, not a later queued thread", () => {
  const requested = requestPeerInput(
    working(),
    scope,
    { kind: "review", key: "review", prompt: "Check drafts" },
    members,
    3
  );
  ...
  state = reduceTaskSession(
    state,
    { type: "steer-thread", actor: "maya", messageId, throughReplyId: first },
    6,
    members
  );
  ...
  state = reduceTaskSession(
    state,
    {
      ...
      actor: "spencer",
      messageId,
      body: "Also check links",
      clientId: "later",
    },
    7,
    members
  );
  const later = state.messages
    .find((m) => m.id === messageId)!
    .annotations!.at(-1)!.id;
  state = reduceTaskSession(
    state,
  ...
  const removed = reduceTaskSession(
    state,
    {
      ...
      actor: "spencer",
      steerId: state.steeringQueue[0].id,
    },
    10,
    members
  );
  assert.equal(
    ...
    removed,
    "removing later queued feedback does not mean Hive addressed it"
  );
});
...
test("queued messages continue in order after completion with a fenced execution grant", () => {
  ...
  assert.equal(
    reduceTaskSession(state, action, 5, members),
    state,
    "never overlaps an active run"
  );
  const idle = applyHiveRunResult(state, result(), 6);
  assert.equal(
    reduceTaskSession(idle, { ...action, steerId: second.id }, 7, members),
    idle,
    "never skips the queue head"
  );
  ...
  assert.equal(
    reduceTaskSession(resumed, { ...action, actor: "maya" }, 9, members),
    resumed
  );
  assert.match(
    buildHiveRunInput(resumed, action, members).steer!,
    /How many people are online/
  );
  const nextIdle = applyHiveRunResult(resumed, result(), 10);
  assert.equal(
    reduceTaskSession(nextIdle, action, 11, members),
    nextIdle,
    "a late client cannot consume the following item"
  );
  assert.equal(
    reduceTaskSession(nextIdle, { ...action, steerId: second.id }, 12, members)
      .activeSteer?.id,
    second.id
  );
});
...
test("automatic message continuation pauses on error and restore, while manual resume remains available", () => {
  ...
  const action = {
    type: "continue-queued-steer",
    actor: "maya",
    steerId: queued.steeringQueue[0].id,
  } as const;
