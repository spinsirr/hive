import { isWorkspaceEvent, type ChatMessage } from "../session/task-session.ts";
...
export function conversationTimelineMessages(
  ...
  return messages.filter((message) => {
    ...
    if (message.role !== "agent" || question?.kind !== "question" || !key)
      return true;
 ...
 */
export function conversationTimelineTurns(
  messages: ChatMessage[]
): ConversationTurn[] {
  const timeline = conversationTimelineMessages(messages);
  const turns: ConversationTurn[] = [];
  const runId = (message: ChatMessage) =>
    message.role === "agent" && message.status !== "error"
      ? (message.interaction?.runId ?? message.id)
      : undefined;
  for (let index = 0; index < timeline.length;) {
    const start = index;
    const key = runId(timeline[index]);
    index += 1;
    while (key && index < timeline.length && runId(timeline[index]) === key)
      index += 1;
    const adjacent = timeline.slice(start, index);
    const root = adjacent.find(
      (message) => !message.interaction && message.id === key
    );
    if (root)
      turns.push({
        message: root,
        requests: adjacent.filter((message) => message !== root),
      });
