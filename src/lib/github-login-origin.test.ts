...
test("alias login moves to the registered callback origin before creating a nonce", () => {
  const destination = canonicalGitHubLoginUrl("https://deployment.example/api/github/login", callback);
...
test("canonical login preserves invitation and repository-authorization parameters", () => {
  ...
  request.searchParams.set("return_to", "/sessions/team-task?invite=signed-fixture");
  ...
  assert.equal(destination.searchParams.get("return_to"), "/sessions/team-task?invite=signed-fixture");
...
test("neither an incoming hostname nor a return path controls the login destination", () => {
  const destination = canonicalGitHubLoginUrl("https://untrusted.example/api/github/login?return_to=https://untrusted.example", callback)!;
  assert.equal(destination.origin, "https://hive.example");
  assert.throws(() => canonicalGitHubLoginUrl("https://deployment.example/api/github/login", ""));
});
...
test("browser Host is used when NextRequest has normalized a loopback alias", () => {
  ...
  assert.equal(canonicalGitHubLoginUrl("https://hive.example/api/github/login", callback, "untrusted.example")?.origin, "https://hive.example");
});
