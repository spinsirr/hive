import { z } from "zod";
import type { CodingRuntime, TaskSessionState } from "./task-session.ts";
...
> & {
  workspace: Pick<
    ...
    "startedAt" | "completedAt" | "restore" | "sandboxName"
  > & {
    agentSession?: {
      id: string;
...
export const isSubagentActive = (task: HiveSubagent) =>
  ["starting", "running", "stopping"].includes(task.status);

/** A lost parent is not proof that its child completed or stopped. */
export function finalizeSubagents(tasks?: HiveSubagent[]) {
  return tasks?.map((task) =>
    isSubagentActive(task) ? { ...task, status: "unconfirmed" as const } : task
  );
}
