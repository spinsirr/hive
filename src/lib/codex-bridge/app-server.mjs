...
async function runNativeTurn({
  ...
  const request = (method, params) =>
    new Promise((resolve, reject) => {
      if (closed) {
        reject(new Error("Codex is no longer running."));
        return;
      }
  ...
  const startThread = () =>
    send({
      id: 1,
      method: resumedThreadId ? "thread/resume" : "thread/start",
      params: {
        ...settings,
  ...
  const prepareThread = () => {
    ...
    if (start.mcpServers?.hive)
      send({
        ...
        params: { extraRoots: [fileURLToPath(new URL(".", import.meta.url))] },
      });
    else startThread();
  };
  const kill = () => {
  ...
  try {
      ...
      if (message.method && message.id != null) {
        if (
          ...
          send({
            ...
            error: {
              code: -32000,
              message: "Reconnect the Codex subscription.",
            },
          });
    ...
    if (!result)
      throw new Error(
        `Codex exited before completing its turn.${stderr ? ` ${stderr}` : ""}`
      );
    if (result.status !== "completed") {
      if (
        ...
        threadId &&
        turnId &&
        isEncryptedHistoryRejection(result.error ?? failure)
      ) {
      ...
      const reason = subscription
        ? subscriptionFailureReason(result.error ?? failure)
        : null;
      if (reason) throw new SubscriptionUnavailable(reason);
      throw new Error(
        result.error?.message ||
          failure?.message ||
          `Codex turn ${result.status}.`
      );
    }
  ...
  // Shutdown failure takes precedence even over a recoverable history mismatch:
  // the outer driver must never retry an unflushed checkpoint. Keep the original
  // turn failure as evidence rather than throwing from finally and discarding it.
  if (processExit.code !== 0 || processExit.signal) {
    throw new Error(
      "Codex did not shut down cleanly; its history checkpoint could not be confirmed.",
      { cause: turnFailure?.error }
    );
  }
  ...
  const usage = {
    inputTokens: {},
    outputTokens: {},
    ...(tokenUsage ? { raw: { codex: tokenUsage } } : {}),
  };
  emit({
    type: "finish-step",
