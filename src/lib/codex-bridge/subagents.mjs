...
export function createSubagents({ request, settings, runId, onChange, timeoutMs = 120_000 }) {
  const tasks = new Map();
  let closed = false;
  let sequence = 0;
  const publicTask = ({ id, runId, requestId, kind, task, status, startedAt, completedAt, threadId, turnId, result }) => ({ id, runId, requestId, kind, task, status, startedAt, completedAt, threadId, turnId, result });
  ...
  async function stop(task) {
    if (terminal.has(task.status) || task.status === "stopping") return publicTask(task);
    task.status = "stopping";
    changed(task);
    if (task.threadId && task.turnId) {
      try { await request("turn/interrupt", { threadId: task.threadId, turnId: task.turnId }); }
      catch { if (!terminal.has(task.status)) { task.status = "unconfirmed"; changed(task); } }
    }
  ...
  async function wait(task, ms) {
    ...
    await new Promise((resolve) => {
      const done = () => { clearTimeout(timer); task.listeners.delete(done); resolve(); };
  ...
  return {
    snapshot,
    ownsThread(threadId) { return [...tasks.values()].some((task) => task.threadId === threadId); },
    async control(input) {
      ...
      if (input.action === "spawn") {
        ...
        if (tasks.size >= MAX_TASKS) throw new Error("At most two subagents may be started per Hive turn.");
        const task = { id: randomUUID(), runId, requestId: input.requestId, kind: input.kind, task: input.task, status: "starting", startedAt: Date.now(), result: "", listeners: new Set(), completedItems: new Set() };
        tasks.set(task.id, task);
        changed(task);
        try {
          const response = await request("thread/start", subagentSettings(settings));
          ...
          const started = task.kind === "review"
            ? await request("review/start", { threadId: task.threadId, delivery: "inline", target: { type: "custom", instructions: task.task } })
            : await request("turn/start", { threadId: task.threadId, input: [{ type: "text", text: task.task, text_elements: [] }] });
          task.turnId = started.turn.id;
          if (task.status === "starting") { task.status = "running"; changed(task); }
          if (closed || task.status === "stopping") {
            if (!terminal.has(task.status)) await request("turn/interrupt", { threadId: task.threadId, turnId: task.turnId });
          } else if (!terminal.has(task.status)) {
            task.timer = setTimeout(() => { void stop(task); }, timeoutMs);
          }
        } catch {
          if (!terminal.has(task.status)) { task.status = task.status === "stopping" ? "unconfirmed" : "failed"; changed(task); }
        }
        return publicTask(task);
      }
      ...
      if (input.action === "read") {
        await wait(task, Math.min(Math.max(Number(input.waitMs) || 0, 0), 10_000));
        return publicTask(task);
      }
      throw new Error("Unsupported subagent operation.");
    },
    notification({ method, params }) {
      const task = [...tasks.values()].find((candidate) => candidate.threadId === params?.threadId);
      ...
      if (!task || terminal.has(task.status) || (task.turnId && eventTurnId && eventTurnId !== task.turnId)) return;
      ...
      if (method === "item/completed") {
        ...
        const text = item.type === "exitedReviewMode" ? item.review : item.type === "agentMessage" && task.kind === "research" ? item.text : "";
        if (text) task.result = [task.result, text].filter(Boolean).join("\n\n").slice(0, MAX_RESULT);
      }
      if (method === "turn/completed") {
        task.status = params.turn.status === "completed" ? "completed" : params.turn.status === "interrupted" ? "stopped" : "failed";
    ...
    async close() {
      ...
      await Promise.all([...tasks.values()].map(async (task) => {
        ...
        while (!terminal.has(task.status) && Date.now() < deadline) await wait(task, deadline - Date.now());
        if (!terminal.has(task.status)) { task.status = "unconfirmed"; changed(task); }
      }));
    },
  };
}
...
export async function serveSubagents(controller, capability) {
  ...
  const server = createServer((socket) => {
    ...
    socket.on("data", (data) => {
      if (received) return;
      body += data.toString();
      if (body.length > 16_384) { socket.destroy(); return; }
      if (!body.includes("\n")) return;
      received = true;
      void (async () => {
        try {
          ...
          socket.end(`${JSON.stringify({ result: await controller.control(input) })}\n`);
