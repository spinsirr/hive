...
import { workspaceReadRequest, workspaceReadResponse } from "./workspace-files.ts";
import { workspaceReadScript } from "./workspace-read-script.ts";

let fixture: string;
let root: string;
function read(relative: string, kind = "file", offset = 0, directory = root) {
  return JSON.parse(execFileSync(process.execPath, ["-e", workspaceReadScript, directory, JSON.stringify({ kind, path: relative, offset })], { timeout: 5_000, maxBuffer: 2_000_000 }).toString());
}
...
before(async () => {
  ...
  for (const name of ["unchanged.ts", "untracked.ts", ".gitignore", ".env.example", "空 格'|$(echo injected).ts"]) {
    await writeFile(path.join(root, name), `// ${name}\nconst message = "你好";\n`);
  }
  await writeFile(path.join(root, "src", "main.tsx"), "export const Main = () => <div>Hello</div>;\n");
  await writeFile(path.join(root, "empty.txt"), "");
  await writeFile(path.join(root, ".env.local"), "TOKEN=DO NOT EXPOSE SECRET");
  await writeFile(path.join(root, ".git", "config"), "DO NOT EXPOSE GIT CREDENTIALS");
  ...
  await writeFile(path.join(root, "invalid-utf8.txt"), Buffer.from([0xff, 0xfe]));
  await writeFile(path.join(root, "huge.txt"), Buffer.alloc(512 * 1024 + 1, 65));
  ...
  await Promise.all(Array.from({ length: 507 }, (_, index) => writeFile(path.join(root, "many", `file-${index}.ts`), "")));
});
...
test("workspace tree includes unchanged, untracked, hidden and nested files, not just diff snapshots", () => {
  ...
  const entries = new Map(result.data.entries.map((entry: { name: string; kind: string }) => [entry.name, entry.kind]));
  for (const name of ["unchanged.ts", "untracked.ts", ".gitignore", ".env.example", "空 格'|$(echo injected).ts"]) assert.equal(entries.get(name), "file");
  ...
  assert.deepEqual(read("src", "directory").data.entries.map((entry: { name: string }) => entry.name), ["main.tsx"]);
...
test("directory pagination exposes every entry beyond the first 500", () => {
  ...
  assert.equal(new Set([...first.entries, ...second.entries].map((entry) => entry.path)).size, 507);
});

test("file reads preserve UTF-8, empty files and shell-like filenames without executing them", async () => {
  for (const name of ["src/main.tsx", "空 格'|$(echo injected).ts", "empty.txt", ".env.example"]) {
    const result = read(name);
    assert.equal(result.status, 200);
    assert.equal(result.data.content, await readFile(path.join(root, name), "utf8"));
...
test("request schema and sandbox both reject absolute, traversal, control and malformed paths", () => {
  for (const name of ["/etc/passwd", "../outside.txt", "src/../../outside.txt", "src//main.tsx", "./unchanged.ts", "src/", "src\\main.tsx", "a\u0000b", "a\nb"]) {
    assert.equal(workspaceReadRequest.safeParse({ kind: "file", path: name }).success, false, name);
...
test("symlinks, credential files and Git internals never leak their contents", () => {
  for (const name of ["linked.txt", "linked-folder/outside.txt", ".env.local", "private.pem", ".git/config"]) {
    const result = read(name);
    assert.equal(result.status, 403, name);
    assert.doesNotMatch(JSON.stringify(result), /DO NOT EXPOSE/);
  }
  assert.equal(read("unchanged.ts", "file", 0, path.join(fixture, "linked-root")).status, 403);
});
