...
test("Claude shares discussion and memory tools without advertising Codex child controls", async () => {
  ...
  try {
    assert.deepEqual((await client.listTools()).tools.map((tool) => tool.name).sort(), ["get_context", "get_presence", "read_thread", "remember_memory", "reply_to_thread", "search_memory"]);
    const result = await client.callTool({ name: "get_context", arguments: {} });
    assert.match(JSON.stringify(result), /claude-code/);
    const unavailable = await client.callTool({ name: "get_presence", arguments: {} });
    assert.match(JSON.stringify(unavailable), /unavailable/);
    assert.equal(JSON.parse((unavailable.content as Array<{ text: string }>)[0].text).onlineCount, null);
...
test(`pending ${kind} content is not readable or shareable as memory by the active agent`, async () => {
  ...
  try {
    ...
    const thread = await client.callTool({ name: "read_thread", arguments: { messageId } });
...
test("multiplayer tools observe only task members and read older discussion without granting execution", async () => {
  ...
  original.annotations = Array.from({ length: 22 }, (_, i) => ({ id: `reply-${i}`, authorId: "spencer", body: `Discussion ${i}`, createdAt: i + 1, status: "open" as const }));
  original.annotations.push({ id: "queued", authorId: "spencer", body: "Unapplied secret instruction", createdAt: 30, status: "queued" });
  context.messages.push(...Array.from({ length: 15 }, (_, i) => ({ ...original, id: `later-${i}`, annotations: [] })));
  let observed = 0;
  const client = new Client({ name: "multiplayer-test", version: "1" });
  await client.connect(new StreamableHTTPClientTransport(new URL("https://hive.test/agent-tools"), {
    fetch: (input, init) => handleHiveMcp(new Request(input, init), scope, {
      read: async () => context,
      reply: async () => { throw new Error("Read tools cannot post replies"); },
      presence: async (sessionId) => { observed++; assert.equal(sessionId, scope.sessionId); return { activeMembers: ["spencer", "spencer", "foreign"], observedAt: 123 }; },
    }, createHiveMemory(undefined)),
  }));
  ...
  try {
    ...
    const second = await read("read_thread", { messageId: original.id, offset: first.nextOffset });
    assert.equal(second.replies.length, 2);
    assert.equal(second.nextOffset, null);
    assert.doesNotMatch(JSON.stringify([first, second]), /Unapplied secret/);
    assert.equal((await client.callTool({ name: "read_thread", arguments: { messageId: "foreign" } })).isError, true);
    assert.equal(context.steeringQueue.length, 0);
    context = { ...context, members: [] };
    assert.equal((await client.callTool({ name: "get_presence", arguments: {} })).isError, true);
