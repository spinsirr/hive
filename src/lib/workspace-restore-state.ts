...
export function workspaceRestoreBlockReason(session: TaskSessionState) {
  ...
  if (session.workspace.restore)
    return session.workspace.restore.status === "unconfirmed"
      ? "Still confirming the restored workspace…"
      : "Restoring workspace…";
  if (isHiveRunActive(session) || session.activeSteer)
    return "Wait for Hive to finish before restoring a checkpoint.";
  return null;
}
...
export function beginWorkspaceRestore(
  ...
  if (
    session.workspace.lastRestore?.id === request.id &&
    session.workspace.lastRestore.snapshotId === request.snapshotId
  )
    return session;
  ...
  if (pending) {
    ...
    if (
      ...
      throw new WorkspaceRestoreError(
        409,
        "A restore is already in progress. Refresh Checkpoints before retrying."
      );
  } else {
    ...
    if (session.version !== request.version)
      throw new WorkspaceRestoreError(
        409,
        "The task changed. Refresh Checkpoints and confirm again."
      );
  }
  const checkpoint = session.workspace.checkpoints?.find(
    (entry) => entry.id === request.snapshotId
  );
  if (
    !checkpoint?.result.agentSession.resumeFrom ||
    checkpoint.result.agentSession.id !== session.workspace.agentSession?.id ||
    checkpoint.result.sandboxName !== session.workspace.sandboxName
  )
    throw new WorkspaceRestoreError(
      409,
      "This snapshot has no matching agent checkpoint and cannot be restored safely."
    );
  return {
    ...session,
    ...
    workspace: {
      ...
      restore: {
        ...
        by: pending?.by ?? member,
        sourceSessionId: pending?.sourceSessionId,
        status: "restoring",
        startedAt: now,
        retryAfter: now + 90_000,
      },
    },
  };
}

export function assertWorkspaceRestoreAttempt(
  session: TaskSessionState,
  operationId: string,
  startedAt: number
) {
  if (
    session.workspace.restore?.id !== operationId ||
    session.workspace.restore.startedAt !== startedAt
  ) {
    throw new WorkspaceRestoreError(
      409,
      "The restore attempt changed. Refresh Checkpoints to see its status."
    );
  }
}

export function recordWorkspaceRestoreSource(
  session: TaskSessionState,
  operationId: string,
  startedAt: number,
  ...
  if (!sourceSessionId)
    throw new WorkspaceRestoreError(
      503,
      "The original workspace could not be identified. Nothing was restored."
    );
  return {
    ...session,
    version: session.version + 1,
    workspace: {
      ...session.workspace,
      restore: { ...operation, sourceSessionId },
    },
  };
}
...
export function completeWorkspaceRestore(
  ...
  return {
    ...
    workspace: {
      ...
      agentSession: {
        ...checkpoint.result.agentSession,
        resumeFrom: coldResumeState(checkpoint.result.agentSession.resumeFrom),
      },
    ...
    messages: [
      ...session.messages,
      {
        ...
        memberId: operation.by.id,
        name: operation.by.name,
        initials: operation.by.initials,
        role: "human",
...
export function failWorkspaceRestore(
  session: TaskSessionState,
  operationId: string
): TaskSessionState {
  ...
  return {
    ...
    workspace: {
      ...session.workspace,
      restore: { ...session.workspace.restore, status: "unconfirmed" },
    },
  };
}
