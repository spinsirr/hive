...
function isRestoredSession(sandbox: Sandbox, operation: WorkspaceRestore) {
  const current = sandbox.currentSession();
  return Boolean(operation.sourceSessionId && current.sessionId !== operation.sourceSessionId &&
    sandbox.status === "running" && sandbox.currentSnapshotId === operation.snapshotId && current.sourceSnapshotId === operation.snapshotId);
}
...
export async function confirmSandboxCheckpoint(session: TaskSessionState, signal: AbortSignal) {
  ...
  return isRestoredSession(await existingWorkspace(session, false, signal), session.workspace.restore);
}
...
export async function restoreSandboxCheckpoint(session: TaskSessionState, signal: AbortSignal, recordSource: (sourceSessionId: string) => Promise<void>) {
  ...
  if (isRestoredSession(sandbox, operation)) return;
  ...
  if (!isRestoredSession(restored, { ...operation, sourceSessionId })) throw new WorkspaceRestoreError(503, "The restored workspace could not be confirmed. Retry this restore shortly.");
