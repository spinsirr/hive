...
import { WORKSPACE_CHECKPOINT_LIMIT, type TaskSessionState, type WorkspaceRestore } from "./task-session.ts";
import { existingWorkspace } from "./workspace-browser.ts";
import { WorkspaceRestoreError } from "./workspace-restore-state.ts";

async function isRestoredSession(sandbox: Sandbox, operation: WorkspaceRestore, signal: AbortSignal) {
  const current = sandbox.currentSession();
  if (!operation.sourceSessionId || current.sessionId === operation.sourceSessionId || current.sourceSnapshotId !== operation.snapshotId) return false;
  if (sandbox.status === "running") return sandbox.currentSnapshotId === operation.snapshotId;
  ...
  return snapshots.some((saved) => saved.id === sandbox.currentSnapshotId && saved.sourceSessionId === current.sessionId &&
    saved.parentId === operation.snapshotId && saved.status === "created" && (!saved.expiresAt || saved.expiresAt > Date.now()));
}

/** Metadata only: never resumes, stops, or rewinds a sandbox. */
export async function confirmSandboxCheckpoint(session: TaskSessionState, signal: AbortSignal) {
  if (!session.workspace.restore?.sourceSessionId) return false;
  return isRestoredSession(await existingWorkspace(session, false, signal), session.workspace.restore, signal);
}

/** Call only after the durable task fence has excluded runs and file readers. */
export async function restoreSandboxCheckpoint(session: TaskSessionState, signal: AbortSignal, recordSource: (sourceSessionId: string) => Promise<void>) {
  ...
  const sourceSessionId = operation.sourceSessionId ?? sandbox.currentSession().sessionId;
  ...
  if (!await isRestoredSession(restored, { ...operation, sourceSessionId }, signal)) throw new WorkspaceRestoreError(503, "The restored workspace could not be confirmed. Retry this restore shortly.");
