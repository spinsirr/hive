import { resolveMember, type TaskSessionState, type TeamMember, type MessageAnnotation } from "./task-session.ts";
...
export function describeHiveContext(context: HiveToolContext) {
  return {
    ...
    discussion: context.messages.filter((message) => !message.status).slice(-12).map((message) => ({
      id: message.id, author: message.name, role: message.role, body: message.body.slice(0, 1600),
      replies: (message.annotations ?? []).slice(-8).map((reply) => ({
...
export function memoryContribution(context: HiveToolContext, messageId: string, replyId?: string) {
  ...
  if (!message || message.status) throw new Error("Choose an existing, completed team contribution.");
