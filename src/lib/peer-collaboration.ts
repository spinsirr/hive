...
export const peerRequestSchema = z
  .object({
...
export function peerRequestKey(message: ChatMessage) {
  const prefix = message.interaction
    ? `peer-${message.interaction.runId}-`
    : undefined;
  return prefix && message.id.startsWith(prefix)
...
export function requestPeerInput(
  ...
  if (
    ...
    !members.some((m) => m.id === request.targetMemberId)
  )
    throw new Error("Choose a task member.");
  ...
  const previous = state.messages.find(
    (message) =>
      ...
      (kind === "question" &&
        ...
        peerRequestKey(message) === request.key)
  );
  if (previous) {
    if (
      previous.body !== request.prompt ||
      previous.interaction?.kind !== kind ||
      previous.interaction?.targetMemberId !== request.targetMemberId ||
      JSON.stringify(previous.interaction?.options) !==
        JSON.stringify(request.options ?? [])
    )
      throw new Error("Request key already used. Read the existing thread.");
    return { session: state, messageId: previous.id };
  }
  if (
    state.messages.filter(
      (message) => message.interaction?.runId === scope.runId
    ).length >= 4
  )
    throw new Error("At most four requests per turn.");
  const message: ChatMessage = {
    threadId:
      kind === "question" ? state.workspace.liveReply?.threadId : undefined,
    id: messageId,
    role: "agent",
    name: "Hive",
    initials: "H",
...
export function canResolvePeerReview(
  state: TaskSessionState,
  message: ChatMessage,
  actor: string,
  revision: string
  ...
  const covered = replies.findIndex(
    (reply) => reply.id === review.coveredThroughReplyId
  );
  return !replies.some(
    (reply, index) =>
      reply.role !== "agent" && index > covered && reply.status !== "steered"
  );
}

/** Bind the review only after actual workspace evidence was collected. */
export function finishPeerReviews(
  state: TaskSessionState,
  ...
  const sourceThreadId =
    source?.kind === "message-thread" || source?.kind === "message-annotation"
      ? source.messageId
      : undefined;
  return messages.map((message) => {
    ...
    if (
      review?.kind !== "review" ||
      !(
        (review.runId === run.id && review.status === "preparing") ||
        message.id === run.threadId ||
        message.id === sourceThreadId
      )
    )
      return message;
    return {
      ...
      interaction: {
        ...review,
        status: succeeded ? "open" : "unavailable",
        revision: succeeded ? run.id : undefined,
        resolved: undefined,
        coveredThroughReplyId:
          succeeded &&
          source?.kind === "message-thread" &&
          source.messageId === message.id
            ? source.throughReplyId
            : review.coveredThroughReplyId,
      },
    };
  });
}
