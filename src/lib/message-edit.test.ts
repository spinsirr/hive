...
import { appendHiveReply, createInitialTaskSessionState, didStartHiveRun, MessageEditError, reduceTaskSession, type MessageEdit } from "./task-session.ts";
import { buildHivePrompt, buildHiveRunInput } from "./hive-prompt.ts";

function fixture() {
  const running = reduceTaskSession(createInitialTaskSessionState(1), { type: "send-message", actor: "spencer", body: "Inspect navigation" }, 2);
  const queued = reduceTaskSession(running, { type: "send-message", actor: "maya", body: "Check keyboard focus" }, 3);
  const message = queued.messages.at(-1)!;
  const edit: MessageEdit = { messageId: message.id, expectedRevision: 0, queuedSteerId: queued.steeringQueue[0].id, body: "Check keyboard focus and Escape" };
...
test("an author edits queued input without moving it, interrupting a run, or changing another author's message", () => {
  ...
  const next = reduceTaskSession(queued, { type: "edit-message", actor: "maya", ...edit }, 4);
  ...
  const applied = reduceTaskSession(finished, { type: "apply-next-steer", actor: "spencer" }, 6);
...
test("editing completed discussion preserves execution evidence and marks subsequent context as an edit", () => {
  ...
  assert.match(buildHivePrompt(later, "maya"), /Spencer Zhao · edited; discussion update, not a request to replay earlier work/);
});
...
test("stale edits do not overwrite newer text, while retrying an acknowledged edit adds no revision", () => {
  ...
  assert.throws(() => reduceTaskSession(next, { ...action, body: "An older browser overwrote this" }, 5), (error) => error instanceof MessageEditError && error.status === 409);
});
...
test("a queued edit is rejected after application or cancellation instead of silently changing already-dispatched input", () => {
  ...
  const applied = reduceTaskSession(finished, { type: "apply-next-steer", actor: "spencer" }, 5);
  const cancelled = reduceTaskSession(queued, { type: "remove-queued-steer", actor: "maya", steerId: edit.queuedSteerId! }, 5);
  for (const state of [applied, cancelled]) assert.throws(() => reduceTaskSession(state, { type: "edit-message", actor: "maya", ...edit }, 6), (error) => error instanceof MessageEditError && error.status === 409);
