...
import { appendHiveReply, applyHiveRunError, conversationMessages, createInitialTaskSessionState, reduceTaskSession } from "./task-session.ts";
...
test("only incremental public text reaches the chat, with distinct text blocks separated", async () => {
  ...
  async function* stream() {
    ...
    assert.deepEqual(received, ["我来"], "must deliver before generation has finished");
    yield { type: "text-delta", text: "检查。" };
    yield { type: "text-end" };
    yield { type: "text-start" };
    yield { type: "text-delta", text: "**检查通过**" };
  }
  assert.equal(await consumeAgentText(stream(), (text) => received.push(text)), "我来检查。\n\n**检查通过**");
  assert.deepEqual(received, ["我来", "我来检查。", "我来检查。\n\n**检查通过**"]);
});

test("provider error and abort events cannot be mistaken for a successful short reply", async () => {
  for (const part of [{ type: "error", error: new Error("provider stopped") }, { type: "abort" }]) {
    const received: string[] = [];
    async function* stream() {
      yield { type: "text-delta", text: "Partial reply" };
      yield part;
    }
    await assert.rejects(consumeAgentText(stream(), (text) => received.push(text)));
...
test("checkpoints coalesce tokens and flush the final pending text exactly once", async () => {
  ...
  const writer = createReplyWriter(async (body, sequence) => { saved.push([body, sequence]); });
  writer.push("H"); writer.push("He"); writer.push("Hello");
...
test("checkpoints are sent during generation, not only at completion", async () => {
  let first!: () => void;
  const checkpoint = new Promise<void>((resolve) => { first = resolve; });
  const saved: string[] = [];
  const writer = createReplyWriter(async (body) => { saved.push(body); first(); }, 1);
  writer.push("first");
  await checkpoint;
...
test("a slow database coalesces pending text instead of building an unbounded write queue", async () => {
  ...
  const writer = createReplyWriter(async (body, sequence) => {
    ...
    if (sequence === 1) { started(); await blocked; }
  }, 1);
  writer.push("first");
  await firstSave;
  ...
  release();
  await finished;
  assert.deepEqual(saved, [["first", 1], ["latest 99", 2]]);
});

function streamingSession() {
  const state = reduceTaskSession(createInitialTaskSessionState(1, "streaming-task"), {
    type: "send-message", actor: "spencer", body: "Clarify the task",
  }, 2);
  state.workspace.liveReply = { id: "agent-reply", body: "Partial reply", sequence: 2, startedAt: 2 };
  return state;
}
...
test("failure preserves already-visible text and ends the typing state", () => {
  ...
  assert.ok(conversationMessages(failed).every((message) => message.status !== "streaming"));
});
