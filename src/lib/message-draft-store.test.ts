...
import { beginMessageSubmission, editMessageDraft, emptyMessageDraft } from "./message-draft.ts";
...
test("draft stores persist identity synchronously and isolate members and tasks", () => {
  ...
  const pending = beginMessageSubmission(editMessageDraft(first.getSnapshot(), "Keep after refresh"), ID);
  ...
  for (const anotherKey of ["hive-draft:v1:task-b:github-101:message", "hive-draft:v1:task-a:github-202:message"]) {
    assert.deepEqual(createMessageDraftStore(anotherKey, () => storage).getSnapshot(), emptyMessageDraft());
  }
...
test("unavailable browser storage does not lose the in-memory draft", () => {
  const store = createMessageDraftStore("draft", () => { throw new Error("Storage disabled"); });
