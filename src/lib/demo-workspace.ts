...
const before = 'export function SettingsNav() {\n  return <nav aria-label="Settings">Settings</nav>;\n}\n';
const after = 'export function SettingsNav() {\n  return <nav aria-label="Settings" tabIndex={0}>Settings</nav>;\n}\n';
const sampleFiles = [
  { path: "README.md", content: "# Sample workspace\n\nInvented files for the Hive demo. No repository is connected.\n" },
  { path: "package.json", content: '{\n  "name": "sample-workspace",\n  "private": true\n}\n' },
  { path: sourcePath, content: before },
];

function result(session: TaskSessionSnapshot["session"], now: number, changed: boolean): HiveRunResult {
  ...
  return {
    ...
    agentSession: { id: session.workspace.agentSession?.id ?? "sample-agent", runtime,
      resumeFrom: { type: "resume-session", specificationVersion: "harness-v1", harnessId: runtime, data: { sample: true } } },
    summary: "Simulated result: the sample workspace is ready for review. No model ran and no repository was changed.",
    diff: changed ? `diff --git a/${sourcePath} b/${sourcePath}\n--- a/${sourcePath}\n+++ b/${sourcePath}\n@@ -1,3 +1,3 @@\n export function SettingsNav() {\n-  return <nav aria-label="Settings">Settings</nav>;\n+  return <nav aria-label="Settings" tabIndex={0}>Settings</nav>;\n }\n` : "",
    files: [{ path: sourcePath, content: changed ? after : before }, ...sampleFiles.filter((file) => file.path !== sourcePath)],
    commands: changed ? [{ command: "npm test", output: "Sample output only — no command executed.\n3 sample checks passed.", exitCode: 0 }] : [],
    changedFiles: changed ? [sourcePath] : [],
  };
}

function repositoryAction(name: string, actor: string): TaskSessionAction {
  return { type: "connect-repository", actor, repositoryId: 1, repositoryName: name, repositoryUrl: `https://github.com/${name}`, repositoryBranch: "main", installationId: 1, visibility: "public", githubUserId: 1, githubLogin: "alex" };
}

function initialSnapshot(task: DashboardTask): TaskSessionSnapshot {
  let session = createInitialTaskSessionState(epoch, `preview-${task.id}`, { title: task.title, createdBy: demoMembers[0].id });
  if (task.repositoryName) {
    session = reduceTaskSession(session, repositoryAction(task.repositoryName, demoMembers[0].id), epoch, demoMembers, models);
  ...
  if (task.archivedAt) session = reduceTaskSession(session, { type: "archive-task", actor: demoMembers[0].id }, task.archivedAt, demoMembers, models);
  return { session, members: demoMembers, activeMembers: demoMembers.map((member) => member.id), typingMembers: [], codingModels: models };
}
...
export function createDemoWorkspace(task: DashboardTask) {
  ...
  const client: HiveClient = {
    ...
    async request(path, init) {
      ...
      if (url.origin !== "https://demo.invalid") return json({ error: "External requests are disabled in the demo." }, 403);
      if (session.archived && method !== "GET") return json({ error: ARCHIVED_TASK_MESSAGE }, 409);
      if (url.pathname === `${base}/files` && method === "GET") {
        const path = url.searchParams.get("path") ?? "";
        const files = session.workspace.files;
        if (url.searchParams.get("kind") === "file") {
          const file = files.find((file) => file.path === path);
          return file ? json({ kind: "file", ...file, bytes: new TextEncoder().encode(file.content).length }) : json({ error: "Sample file not found." }, 404);
        }
        const prefix = path ? `${path}/` : "";
        const entries = new Map<string, { name: string; path: string; kind: string }>();
        ...
        return json({ kind: "directory", path, entries: [...entries.values()], nextOffset: null });
      }
      if (url.pathname === `${base}/checkpoints`) {
        if (method === "GET") return json({ checkpoints: (session.workspace.checkpoints ?? []).map((entry) => ({ id: entry.id, createdAt: entry.createdAt, sizeBytes: 0, current: entry.id === session.workspace.lastRestore?.snapshotId || entry.createdAt === session.workspace.completedAt, restorable: true })), retentionCount: 3, version: session.version, blockedReason: workspaceRestoreBlockReason(session), restore: null });
        if (method === "POST") {
          try {
            const request = restoreWorkspaceRequest.parse(JSON.parse(String(init?.body)));
            const restoring = beginWorkspaceRestore(session, request, member);
            publish({ ...snapshot, session: completeWorkspaceRestore(restoring, request.id) });
            return json(snapshot);
          } catch (error) { return json({ error: error instanceof Error ? error.message : "Sample restore failed." }, 409); }
        }
      }
      if (url.pathname === "/api/github/repositories" && url.searchParams.get("session_id") === session.sessionId) {
        if (method === "GET") return json({ repositories: [{ id: 1, name: "sample-team/website", defaultBranch: "main", visibility: "public" }] });
        if (method === "POST" && JSON.parse(String(init?.body)).repositoryId === 1) {
          const connected = reduceTaskSession(session, repositoryAction("sample-team/website", member.id), Date.now(), demoMembers, models);
          publish({ ...snapshot, session: { ...connected, workspace: { ...connected.workspace, files: sampleFiles } } });
          return json({ connected: true });
        }
      }
      // Fail closed: new production endpoints need an explicit demo implementation.
      return json({ error: "This action is unavailable in the local demo." }, 403);
    },
  };
  return {
    ...
    setTyping: () => {},
    setMember: (id: string) => { member = demoMembers.find((candidate) => candidate.id === id) ?? member; },
    ...
    finishRun(runId: string) {
      ...
      } else session = appendHiveReply(session, "Simulated reply: discuss the outcome with a teammate, or attach the sample repository to explore Files and review. No model was called.");
      publish({ ...snapshot, session });
    },
  };
}
