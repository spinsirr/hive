...
import { beginWorkspaceRestore } from "../workspace/workspace-restore-state.ts";
import { createDemoWorkspace, demoMembers } from "../demo/demo-workspace.ts";
import { demoTasks, demoTaskHref } from "../demo/ui-demo.ts";
...
test("team archive and restore preserve work, are idempotent and never grant execution", () => {
  ...
  assert.equal(
    reduceTaskSession(
      archived,
      { type: "restore-task", actor: "outsider" },
      3,
      members
    ),
    archived
  ...
  assert.equal(
    reduceTaskSession(
      initial,
      { type: "archive-task", actor: "outsider" },
      3,
      members
    ),
    initial
  );
  const restored = reduceTaskSession(
    archived,
    { type: "restore-task", actor: other },
    4,
    members
  );
...
test("archives reject every task mutation and checkpoint rollback until restored", () => {
  ...
  const actions: Record<TaskSessionAction["type"], boolean> = {
    ...
    "restore-task": true,
    "rename-task": false,
    "select-harness": false,
    "set-coding-effort": false,
    "recover-stalled-run": false,
    reset: false,
    "connect-repository": false,
    "send-message": false,
    "edit-message": false,
    "annotate-code": false,
  ...
  for (const [type, allowed] of Object.entries(actions)) {
    ...
    assert.equal(
      Boolean(taskActionBlockReason(archived, action)),
      !allowed,
      type
    );
    if (!allowed)
      assert.equal(
        reduceTaskSession(archived, action, 3, members),
        archived,
        type
      );
  }
...
test("archived demo uses production read-only rules, keeps files visible and supports teammate restore", async () => {
  ...
  assert.equal(
    (await demo.client.request(`${base}/checkpoints`, { method: "POST" }))
      .status,
    409
  );
  assert.equal(
    (
      await demo.client.request(
        `/api/github/repositories?session_id=${initial.session.sessionId}`,
        { method: "POST", body: '{"repositoryId":1}' }
      )
    ).status,
    409
  );
