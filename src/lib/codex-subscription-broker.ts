  ...
  const payload = Buffer.from(
    JSON.stringify({
      ...
      "https://api.openai.com/auth": {
        chatgpt_account_id: auth.chatgptAccountId,
      },
    })
  ).toString("base64url");
  return {
    accessToken: `e30.${payload}.${Buffer.from(randomUUID()).toString("base64url")}`,
    chatgptAccountId: auth.chatgptAccountId,
  };
}

/** No account management, cloud tasks, arbitrary paths, or off-host credential use. */
export function codexOAuthTransformations(
  auth: CodexAccess,
  placeholder: CodexAccess
      ...
      headers: [
        {
          ...
          value: { exact: `Bearer ${placeholder.accessToken}` },
        },
      ],
    },
    transform: {
