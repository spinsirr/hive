...
export function beginMessageSubmission(
  ...
  return {
    ...
    status: "sending",
    submission:
      draft.submission?.body === body ? draft.submission : { clientId, body },
  };
}

export function acknowledgeMessageSubmission(
  draft: MessageDraft,
  clientId: string
): MessageDraft {
...
export function failMessageSubmission(
  draft: MessageDraft,
  clientId: string
): MessageDraft {
  if (draft.submission?.clientId !== clientId) return draft;
...
export function restoreMessageDraft(value: string | null): MessageDraft {
  try {
    ...
    return {
      ...
      submission.body === saved.body.trim()
