...
test("the installed Claude bridge preserves the captured native MCP content array", async () => {
  ...
  const source = await readFile(new URL("../../node_modules/@ai-sdk/harness-claude-code/src/bridge/create-emit-stream-event.ts", import.meta.url), "utf8");
  const { outputText } = ts.transpileModule(source, { compilerOptions: { module: ts.ModuleKind.ESNext, target: ts.ScriptTarget.ES2022 } });
  const { createClaudeStreamEventState, createEmitStreamEvent } = await import(`data:text/javascript;base64,${Buffer.from(outputText).toString("base64")}`);
  const events: { type: string; toolName?: string; result?: unknown; delta?: string }[] = [];
  const emit = createEmitStreamEvent({ state: createClaudeStreamEventState(),
    emit: (event: typeof events[number]) => events.push(event),
    emitWarning: () => {}, emitTerminalError: () => {}, onCompactionBoundary: () => {}, toCommonName: (name: string) => name,
  });
  const content = [{ type: "text", text: JSON.stringify(receipt) }];
  emit({ type: "assistant", message: { content: [{ type: "tool_use", id: "question-tool", name: "mcp__hive__request_input", input: {} }] } });
  emit({ type: "user", tool_use_result: content, message: { content: [{ type: "tool_result", tool_use_id: "question-tool", content }] } });
  emit({ type: "stream_event", event: { type: "content_block_start", index: 0, content_block: { type: "text" } } });
  emit({ type: "stream_event", event: { type: "content_block_delta", index: 0, delta: { type: "text_delta", text: "Detailed" } } });
  assert.deepEqual(events.find(event => event.type === "tool-result")?.result, content);
  // Harness maps result -> output and delta -> text in its public fullStream.
  const stream = parts(events.map(event => ({ ...event, output: event.result, text: event.delta })));
  assert.equal(await consumePlanningText(stream, () => assert.fail("Do not publish the invented answer")), "");
});
...
test("a confirmed planning question ends public prose, not the native stream", async () => {
  for (const toolName of ["request_input", "mcp__hive__request_input"]) {
    for (const output of [receipt, { content: [{ type: "text", text: JSON.stringify(receipt) }] }, [{ type: "text", text: JSON.stringify(receipt) }]]) {
      let drained = false;
