...
test("explicit platform credentials are independent of task, owner and repository", () => {
  ...
  assert.equal(claudeSubscriptionToken({ CLAUDE_CODE_OAUTH_TOKEN: token, ANTHROPIC_API_KEY: "host-key" }), undefined);
  assert.throws(() => claudeSubscriptionToken({ HIVE_CLAUDE_OAUTH_TOKEN: "sk-ant-api03-wrong-type" }), (error: unknown) => {
    assert.ok(error instanceof Error); assert.doesNotMatch(error.message, /sk-ant/); return true;
  });
});

test("every task gets the same native model menu; missing credentials never select paid fallback", () => {
  for (const env of [{ HIVE_CODEX_AUTH_SECRET: "configured" }, { HIVE_CLAUDE_OAUTH_TOKEN: token }]) {
    assert.equal(usesPlatformSubscriptions(env), true);
    const models = platformCodingModels({ ...env, HIVE_CODEX_MODEL: "openai/gpt-5.1-codex-mini" });
  ...
  assert.equal(usesPlatformSubscriptions({ AI_GATEWAY_API_KEY: "api-only-installation" }), false);
});
