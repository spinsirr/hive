...
import { repositoryDirectory, resolvePersistentSandboxName } from "./hive-sandbox.ts";
...
const commandResponse = z.union([
  ...
  z.object({ status: z.union([z.literal(400), z.literal(403), z.literal(404), z.literal(413), z.literal(415), z.literal(500)]), error: z.string() }),
]);

export async function readWorkspace(session: TaskSessionState, input: WorkspaceReadRequest, signal: AbortSignal) {
  const request = workspaceReadRequest.safeParse(input);
  if (!request.success) throw new WorkspaceReadError(400, "Invalid workspace path.");
  const sandbox = await existingWorkspace(session, true, signal);
  const root = path.posix.join(sandbox.currentSession().cwd, repositoryDirectory(session.repository!.url));
  ...
  if (result.exitCode !== 0) throw new WorkspaceReadError(503, "Workspace could not be read. Try again.");
  const response = commandResponse.parse(JSON.parse(await result.stdout({ signal })));
  if (response.status !== 200) throw new WorkspaceReadError(response.status, response.error);
...
async function existingWorkspace(session: TaskSessionState, resume: boolean, signal: AbortSignal) {
  ...
  try {
    ...
    sandbox = await Sandbox.get({ name: resolvePersistentSandboxName(session, agentId), resume, signal });
  } catch {
    throw new WorkspaceReadError(503, "Workspace is unavailable. Try again shortly.");
  }
  if (sandbox.tags?.session && sandbox.tags.session !== session.sessionId) {
    throw new WorkspaceReadError(403, "Workspace does not belong to this task.");
  }
  return sandbox;
}
