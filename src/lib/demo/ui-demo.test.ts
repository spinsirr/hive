...
import { dashboardTasks } from "../tasks/task-dashboard.ts";
import {
  demoLoadedAt,
  demoTaskHref,
  demoTasks,
...
test("UI demo fixtures keep all sample tasks with invented identifiers and an unattached example", () => {
  ...
  assert.equal(
    new Set(demoTasks.map((task) => task.id)).size,
    demoTasks.length
  );
  assert.ok(
    demoTasks.every(
      (task) =>
        ...
        (!task.repositoryName || task.repositoryName.startsWith("sample-team/"))
    )
  );
});
...
test("each sample has a distinct, reloadable demo route and task-specific conversation", () => {
  ...
  assert.equal(
    new Set(demoTasks.map((task) => demoTaskMessages(task)[0].body)).size,
    demoTasks.length
  );
