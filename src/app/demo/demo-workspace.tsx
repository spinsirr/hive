...
export function DemoWorkspace({ task }: { task: DashboardTask }) {
  ...
  return (
    <HiveClientContext value={demo.client}>
      <HiveWorkspaceView
        ...
        connection={{
          ...
          receiveSnapshot: demo.receiveSnapshot,
          setTyping: demo.setTyping,
          syncing: false,
          syncError: false,
        }}
        notice={
          <div
            className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-border bg-muted/40 px-4 py-1.5 text-xs text-muted-foreground"
            aria-label="Demo controls"
          >
            <span>
              Demo · Sample data and simulated results · No model calls
            </span>
            <div className="flex items-center gap-1">
              <span className="mr-1">You are</span>
              {demoMembers.map((candidate) => (
                <Button
                  key={candidate.id}
                  aria-pressed={candidate.id === member.id}
                  className="h-7 px-2 text-xs"
                  onClick={() => chooseMember(candidate.id)}
                  size="sm"
                  variant={candidate.id === member.id ? "secondary" : "ghost"}
                >
                  {candidate.shortName}
                </Button>
              ))}
