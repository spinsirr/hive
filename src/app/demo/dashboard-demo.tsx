  ...
  return (
    <>
      <aside
        ...
        className="flex flex-wrap items-center justify-between gap-x-5 gap-y-2 border-b border-[#e8e8e8] bg-[#f4f4f4] px-5 py-2 text-xs text-[#737373] sm:px-8"
      >
        <p>
          <span className="font-medium text-[#171717]">UI demo</span>
          <span className="mx-2" aria-hidden="true">
            ·
          </span>
          Sample data. No connected repository or agent.
        </p>
      ...
      <TaskDashboard
        ...
        onArchiveTask={async (id, archived) => {
          setTasks((current) =>
            current.map((task) =>
              task.id === id
                ? {
                    ...task,
                    archivedAt: archived ? demoLoadedAt : null,
          ...
          setNotice(
            archived
              ? "Task archived for the sample team."
              : "Sample task restored."
          );
        }}
