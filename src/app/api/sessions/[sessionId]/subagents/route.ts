...
import { isTaskSessionMember, withTaskSubagentControl } from "@/lib/task-session-store";
import { controlSubagent } from "@/lib/subagent-control";

export const runtime = "nodejs";
export const maxDuration = 30;
const headers = { "Cache-Control": "private, no-store" };
const stopRequest = z.object({ id: z.string().uuid(), runId: z.string().min(1).max(120) }).strict();
...
export async function POST(request: NextRequest, context: { params: Promise<{ sessionId: string }> }) {
  ...
  if (body.length > 2048) return NextResponse.json({ error: "Invalid request" }, { status: 400, headers });
  let input;
  try { input = stopRequest.parse(JSON.parse(body)); }
  ...
  try {
    ...
    const task = await withTaskSubagentControl(sessionId, (session) => controlSubagent(session, input.runId, { action: "stop", id: input.id }, signal));
    return NextResponse.json({ task }, { headers });
  } catch {
    return NextResponse.json({ error: "Stop could not be confirmed. Check the task's current state before retrying." }, { status: 409, headers });
  }
}
